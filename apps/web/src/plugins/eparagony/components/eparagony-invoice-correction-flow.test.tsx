/**
 * EparagonyInvoiceCorrectionFlow tests (#3193)
 *
...
import { EparagonyInvoiceCorrectionFlow } from './eparagony-invoice-correction-flow';
...
describe('EparagonyInvoiceCorrectionFlow', () => {
  it('renders the form header and invoice reference', async () => {
    renderWithProviders(
      <EparagonyInvoiceCorrectionFlow
        invoice={makeInvoice()}
    ...
    expect(await screen.findByText(/FV 101\/TEST\/2026/i)).toBeDefined();
  });
  ...
  it('submits issueCorrection with reason and line data on form submit', async () => {
    ...
    await waitFor(() => {
      expect(issueCorrection).toHaveBeenCalledWith('ol_invoice_test', {
        ...
        idempotencyKey: expect.stringMatching(/^eparagony-corr-/),
      });
    });
  ...
  it('reports a line error when neither quantity nor price is provided', async () => {
