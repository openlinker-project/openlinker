/**
 * Infakt Credentials Panel
 *
 * Plugin-owned credentials affordance for inFakt connections — rotates the
 * stored API key in place via PUT /credentials. Owns its own toggle state,
 * mutation, and toast feedback; the parent `EditConnectionForm` renders this
 * only when the plugin contributes it. When the credential is env-backed
 * (`credentialsBacked === false`) the panel shows a read-only affordance
 * instead. Mirrors `ErliCredentialsPanel` (single `apiKey`, no auth-type
 * enum).
 *
...
export function InfaktCredentialsPanel({ connection }: { connection: Connection }): ReactElement {
  ...
  const onRotate = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      showToast({
        ...
        description: 'The new inFakt API key is now in use.',
      });
      setApiKey('');
      setShowRotate(false);
    } catch {
