 ...
 * (via renderWithProviders, so usePlatform('subiekt-nexo') resolves the registered
 ...
 *
 * @module plugins/subiekt-nexo/components
 */
...
describe('SubiektStructuredSection', () => {
  ...
  it('routes the payment-method Select change to subiektNexoPaymentMethod', () => {
    const syncStructuredToJson = vi.fn();
    renderWithProviders(<Harness syncStructuredToJson={syncStructuredToJson} />);
    ...
    expect(syncStructuredToJson).toHaveBeenCalledWith('subiektNexoPaymentMethod', 'transfer');
  });
  ...
  it('offers an explicit "Not set" payment option and hides the bank select when unset (tri-state)', () => {
    ...
    renderWithProviders(<Harness defaultValues={{ subiektNexoPaymentMethod: '' }} />);
  ...
  it('hides the bank-account select when payment method is Cash', () => {
    bankAccountsResult.data = [account({})];
    renderWithProviders(<Harness defaultValues={{ subiektNexoPaymentMethod: 'cash' }} />);
    expect(screen.queryByLabelText('Bank account for Transfer invoices')).not.toBeInTheDocument();
  });
  ...
  it('shows a flat bank-account list and NO payer warning for a single-payer install', () => {
    ...
    renderWithProviders(<Harness defaultValues={{ subiektNexoPaymentMethod: 'transfer' }} />);
  ...
  it('fires the set-default mutation when a non-default account is picked', () => {
    ...
    renderWithProviders(
      <Harness
        syncStructuredToJson={syncStructuredToJson}
        defaultValues={{ subiektNexoPaymentMethod: 'transfer' }}
      />,
    );
    ...
    expect(syncStructuredToJson).toHaveBeenCalledWith('subiektNexoBankAccountId', '2');
  ...
  it('routes the cash-register Select change to subiektNexoStanowiskoKasoweId', () => {
    cashRegistersResult.data = [
    ...
    expect(syncStructuredToJson).toHaveBeenCalledWith('subiektNexoStanowiskoKasoweId', '100065');
  });
});
