...
function csvCell(value: string | number | boolean | null): string {
  ...
  // The guard is scoped to STRING cells only — `availableQuantity` /
  // `reservedQuantity` are legitimately negative numbers, and applying the
  // pattern to every cell exported `-3` as the text `'-3` in the two columns
  // an operator is exporting the file to reconcile (#3264 review). The
