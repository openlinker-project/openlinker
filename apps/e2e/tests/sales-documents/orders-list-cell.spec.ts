...
/**
 * Scope a locator to the row/card for one seeded order.
 *
 * #2807 review — a bare `getByRole('button', { name: ariaLabel })` is
 * ambiguous the moment ANOTHER suite's fixture (e.g. the settings
 * market-list seed's unconfigured FI/SE/NO orders) is also live on `/orders`
 * and happens to render the same generic word ("No document: No routing").
 */
function rowFor(page: Page, orderId: string): Locator {
  // The visible id is middle-elided (`shortenId`, `EntityLabel`); the full,
  // unshortened value survives only in the `title` attribute of the
  // `.entity-label__id` chip (`entity-label.tsx`), never as literal text
  // content — so scope by that attribute, not `hasText`.
  return page.locator('tr, li').filter({ has: page.locator(`[title="${orderId}"]`) });
}

function triggerFor(page: Page, orderId: string, ariaLabel: string): Locator {
  return rowFor(page, orderId).getByRole('button', { name: ariaLabel, exact: true });
}
...
test.describe('sales documents: /orders money-cluster document line (#2563 M10)', () => {
    ...
    test.describe(`at ${viewport.label}`, () => {
      ...
      for (const state of STATES) {
        test(`renders "${state.ariaLabel}" and its popover`, async ({ page }) => {
          ...
          const trigger = triggerFor(page, SEED_ORDER_IDS[state.key], state.ariaLabel);
  ...
  test.describe('keyboard operability (representative states)', () => {
    ...
    test('Enter opens the popover for a finished (done-tone) document', async ({ page }) => {
      await loginAndGoToOrders(page);
      const trigger = triggerFor(page, SEED_ORDER_IDS.invoiceIssued, 'Invoice: Issued');
      await trigger.focus();
      await expect(trigger).toBeFocused();
    ...
    test('Space opens the popover for an in-flight (progress-tone) document', async ({
      ...
      await loginAndGoToOrders(page);
      const trigger = triggerFor(page, SEED_ORDER_IDS.fiscalRegistering, 'Fiscal receipt: Registering');
      await trigger.focus();
  ...
  test('never renders the mockup\'s platform-named words for authority states', async ({
    ...
    await expect(triggerFor(page, SEED_ORDER_IDS.invoiceAuthorityRejected, 'Invoice: Authority rejected')).toBeVisible();
    await expect(triggerFor(page, SEED_ORDER_IDS.invoiceAtAuthorityDuplicate, 'Invoice: At authority')).toBeVisible();
    ...
    await expect(triggerFor(page, SEED_ORDER_IDS.fiscalNotIssued, 'Fiscal receipt: Not issued')).toBeVisible();
  ...
  test('the duplicate-document warning names the other connection, not just a count', async ({
    ...
    await loginAndGoToOrders(page);
    const trigger = triggerFor(page, SEED_ORDER_IDS.invoiceAtAuthorityDuplicate, 'Invoice: At authority');
