...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: OUT + '/C1-pre-address.png', fullPage: true });

    const addBtns = page.getByRole('button', { name: 'Add new address' });
    console.log('count', await addBtns.count());
  } finally {
    await browser.close();
  }
})();
