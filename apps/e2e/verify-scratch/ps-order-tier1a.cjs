...
(async () => {
  ...
  try {
    const context = await browser.newContext({ storageState: OUT + '/../ps-admin-state.json', viewport: { width: 1700, height: 1400 } });
    ...
    await page.screenshot({ path: OUT + '/PSA3-after-choose-customer.png', fullPage: true });
  } finally {
    await browser.close();
  }
})();
