/**
 * Marketplace Offer Status Sync Handler Tests
 *
 * Unit tests for MarketplaceOfferStatusSyncHandler (#816): scan-offset cursor
 * read/parse/advance, default cursor key + limit, ok outcome, and error
 * wrapping.
 *
...
import type { OfferStatusSyncResult } from '@openlinker/core/listings';
...
describe('MarketplaceOfferStatusSyncHandler', () => {
  ...
  const baseResult: OfferStatusSyncResult = {
    scanned: 1,
    updated: 1,
    transitioned: 0,
    notFound: 0,
  ...
  beforeEach(() => {
    offerStatusSync = { sync: jest.fn().mockResolvedValue(baseResult) };
    cursorRepository = {
  ...
  const createJob = (payload: Record<string, unknown>): SyncJob => ({
    id: 'job-id',
    jobType: 'marketplace.offer.statusSync' as unknown as SyncJob['jobType'],
  ...
  it('starts at offset 0 with the default cursor key when no cursor is stored', async () => {
    ...
    expect(cursorRepository.get).toHaveBeenCalledWith('connection-1', 'allegro.offerStatus.scanOffset');
    expect(offerStatusSync.sync).toHaveBeenCalledWith('connection-1', { limit: 25, offset: 0 });
  ...
  it('parses the stored numeric offset and passes it to the service', async () => {
    const job = createJob({ schemaVersion: 1, limit: 25, cursorKey: 'allegro.offerStatus.scanOffset' });
    cursorRepository.get.mockResolvedValue('30');

    await handler.execute(job);

    expect(offerStatusSync.sync).toHaveBeenCalledWith('connection-1', { limit: 25, offset: 30 });
  });
  ...
  it('defaults limit to 100 when the payload limit is missing or invalid', async () => {
    ...
    expect(offerStatusSync.sync).toHaveBeenCalledWith('connection-1', { limit: 100, offset: 0 });
  });
  ...
  it('treats a non-numeric stored cursor as offset 0', async () => {
    ...
    expect(offerStatusSync.sync).toHaveBeenCalledWith('connection-1', { limit: 10, offset: 0 });
  });
  ...
  it('wraps service failures in SyncJobExecutionError', async () => {
    const job = createJob({ schemaVersion: 1, limit: 10 });
    offerStatusSync.sync.mockRejectedValue(new Error('boom'));
