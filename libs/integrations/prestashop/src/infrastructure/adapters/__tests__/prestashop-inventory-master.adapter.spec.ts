...
describe('PrestashopInventoryMasterAdapter', () => {
  ...
  describe('adjustInventory', () => {
    ...
    it('should target the combination row when variantId names one', async () => {
      ...
      // The read is PAGED since #2616, so the call carries a deterministic sort
      // plus limit/offset beside the filter. The filter is what this asserts.
      expect(mockHttpClient.listResources).toHaveBeenCalledWith(
        'stock_availables',
        expect.objectContaining({
    ...
    describe('idempotency', () => {
      ...
      function adapterWithCache(): PrestashopInventoryMasterAdapter {
        return new PrestashopInventoryMasterAdapter(
          ...
          inventoryMapper,
          connection,
          // packResolver is not what this block exercises; the cache is the
          // trailing parameter, so it must be named past an explicit undefined.
          undefined,
          cache
        );
      }
