 ...
 *      string ('invoice'->'faktura', 'receipt'->'paragon'). Anything outside
 *      the supported set throws `SubiektUnsupportedDocumentTypeError`.
 *
...
/** Scheme that triggers `faktura` when present with a non-empty value. */
export const PL_NIP_SCHEME = 'pl-nip';

/**
 * Bridge-native document-kind strings. The bridge resolves these to a Subiekt
 * document; #752 owns the authoritative values.
 */
const NEUTRAL_TO_BRIDGE_DOCUMENT_TYPE: Readonly<Record<'invoice' | 'receipt', string>> = {
  invoice: 'faktura',
  receipt: 'paragon',
};
...
/**
 * Map a neutral document type to the bridge-native Subiekt string.
 * @throws SubiektUnsupportedDocumentTypeError when `neutral` is outside
 *   `{invoice, receipt}`.
 */
export function toBridgeDocumentType(neutral: string): string {
