...
export class SubiektInventoryMasterAdapter implements InventoryMasterPort {
  ...
  async getInventory(productId: string, locationId?: string): Promise<Inventory> {
    const { towarSymbol, positions } = await this.readPositions(productId);
    const filtered = locationId
      ? positions.filter((p) => String(p.magazynId) === locationId)
      : positions;
  ...
  async adjustInventory(adjustment: InventoryAdjustment): Promise<InventoryAdjustmentResult> {
    ...
    try {
      ...
      const { positions } = await this.readPositions(adjustment.productId);
      const inventory = this.toNeutralInventory(adjustment.productId, towarSymbol, positions);
  ...
  async getAvailableQuantity(productId: string, locationId?: string): Promise<number> {
    const { positions } = await this.readPositions(productId);
    const filtered = locationId
      ? positions.filter((p) => String(p.magazynId) === locationId)
      : positions;
