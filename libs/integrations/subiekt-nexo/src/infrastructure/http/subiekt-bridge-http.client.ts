 ...
 *
 * @module libs/integrations/subiekt-nexo/src/infrastructure/http
 */
...
export class SubiektBridgeHttpClient implements SubiektBridgeClient {
   ...
   * shown a green tick for a configuration that could never work.
   *
   * `/api/bank-accounts` is the probe instead because the auth middleware sits
   * in FRONT of every `/api/*` route, so any answer other than 401/403 proves
   * authorization passed — and it is a plain read with no side effects, which a
   * probe that may run on a schedule has to be.
   *
  ...
  private async request<T>(
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      // the body" rule, so the operator sees WHICH auth problem they have. The
      // rule that comment was protecting (never surface the token) is kept by
      // `redactToken`, which is stronger than not reading at all: it holds even
      // for a bridge that echoes the credential back.
