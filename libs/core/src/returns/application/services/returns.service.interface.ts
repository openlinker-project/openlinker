...
export interface IReturnsService {
   ...
   *
   * Every downstream flow calls this — `restock`, `refund` and `invoice_correction` (see
   * `ReturnDownstreamTriggerValues`) — and none writes its own orphan check. Three call
   * sites each free to spell `internalOrderId === null` are three chances to forget, and
