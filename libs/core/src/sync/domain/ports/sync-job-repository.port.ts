...
export interface SyncJobRepositoryPort {
   ...
   * succeeded) rather than `createdAt` (enqueue time). This is the precise
   * "last successful ingestion" signal — `findMany` orders by `createdAt`
   * only, which is enqueue time, not completion time (#1982).
   *
   * @param connectionId - Connection UUID
