...
**The two mechanisms behind all nine instances, and the sentence that covers cases neither list enumerates** (#2840/#2842, 2026-09-08/09). By the end of one branch there were nine, and listing nine invites memorising nine. They are two:

- **A probe that cannot identify its own subject.** It matches its own command line (`pgrep -f`/`pkill -f`), its own name (`ps aux | grep '[s]cenario-name'`), its own log file (two rungs sharing a filename keyed on a parameter value), or a container's mere *presence* rather than the measurement window.
- **A probe that reads a proxy when an authority exists.** `tee`'s exit status instead of the command's; `grep -c`'s exit code instead of its printed count; a results **file** consulted to decide whether a shared stand was free when the **lock** was the authority - and the lock then refused the run, i.e. the system worked and the operator consulted the proxy.
...
**Problem**: each unbounded list read had its own paging loop, and every one of them ended the same way: stop after N pages (or after one), return what was read. The caller receives a short array, and **nothing anywhere distinguishes "that is the whole collection" from "that is as far as I looked"**. The consequences are silent and land far away - a pack whose components were half-read is priced wrong, a filter that returned page one links the wrong products, and a partial enumeration feeding anything that reasons about *absence* turns a paging bound into a false deletion (which is [ADR-048](./architecture/adrs/048-incremental-catalog-replication.md) decision 2's whole subject, and exactly what its deleted `MAX_PAGES` guard did one layer up: it warned `pagination may be truncated` and then returned `outcome: 'ok'`). A `truncated: true` flag beside the data is the same defect wearing a hat - it is only as good as every caller checking it, and the callers that most need to are the ones that read a shorter array as a smaller world.

**Rule**: page to the end of the collection or throw. End paging only on a **short page**; filling the whole budget is an error, not a completion. Route every unbounded list read through the shared helper (`readAllPrestashopResourcePages` / `findAcrossPrestashopResourcePages` in `libs/integrations/prestashop/src/infrastructure/http/prestashop-paged-read.ts`), which throws `PrestashopTruncatedReadException` naming the resource, connection and budget. Two related traps come free with it and are worth knowing separately: an offset scan with **no `ORDER BY` has no tiling guarantee at all** (MySQL may repeat a row and never return another - the #2605 order-feed failure), so a paged resource read injects `sort: ['id_ASC']`; and the page size is a **per-connection configuration value**, so a budget expressed in pages means something different on each shop - express it in rows and convert. A call site that must degrade on other errors still has to rethrow the truncation explicitly (the pack resolver does).

**Applies to**: `libs/integrations/prestashop/src/**` - any list read over a paginated resource. The defect class is not PrestaShop-specific; any adapter growing its own paging loop is due for it.

**Source**: #2608 (the policy + helper), #2598, #2616, #2605; [ADR-068](./architecture/adrs/068-page-to-the-end-or-fail-loudly.md).

## In PHP, `empty()` is true for a stored `"0"` - test for absence explicitly, and remember `id_shop = 0` means global

**Context**: two independent instances in the same epic's PrestaShop module. The 1.10.0 upgrade seeds two operator-visible defaults (#2652), and the 1.5.0 settings migration partitions configuration rows into global and per-shop (#2602).

**Problem**: PHP's `empty()` is true for `0`, `"0"`, `""`, `null` and `false` alike, so an upgrade guard written as `if (empty($stored))` treats a **deliberately chosen zero** as an unset value - and, because a module upgrade re-runs, it silently resets that operator's choice on every version bump, with no error and nothing in the logs. The sibling case is the same shape in someone else's data model: the settings migration selected shop-scoped rows with `id_shop IS NOT NULL`, but PrestaShop core treats `id_shop = 0` as **global** (its own `sqlRestriction` defends against the zero, and legacy dumps, SQL imports and third-party writes all produce such rows). On such a database the migration picked the global row up as per-shop, wrote the global value into that same row, and then deleted it - so the webhook signing secret ended up in no row at all, webhooks stopped authenticating, and there was nothing left to recover the value from.

**Rule**: in PHP, never use `empty()` to mean "not set". Test the states you actually mean - `$stored === false || $stored === null || $stored === ''` for a `Configuration::get*` read, which returns `false` for an absent key. And before writing a predicate over somebody else's column, check what **their** code treats as the sentinel: for PrestaShop's `id_shop` / `id_shop_group`, global is `NULL` **or** `0`, so the predicate is `COALESCE(id_shop, 0) <> 0 OR COALESCE(id_shop_group, 0) <> 0`. Pin both with a test against real MySQL - a unit test with a mocked configuration store cannot observe either bug.

**Applies to**: `apps/prestashop-module/openlinker/**`, especially `upgrade/upgrade-*.php` (which re-run) and any migration partitioning `ps_configuration`; more generally any PHP guard over a numeric setting.

**Source**: #2652 (`upgrade-1.10.0.php`), #2602 (the `id_shop = 0` predicate, pinned by two MySQL tests that both fail under the old one).

## A job's lane is chosen by the cost of starving it, which is a property of its TRIGGER, not of its type

**Context**: `master.product.syncByExternalId` was registered in the `realtime` lane ([ADR-050](./architecture/adrs/050-workload-isolation-concurrency-lanes.md)). It is a webhook-driven single-product sync that someone is waiting on - and it was *also* the child the catalogue sweep enqueued a budget wide, every tick (#2594).

**Problem**: one job type carried two different costs of starvation, so the lane assignment was wrong in **both** directions at once. A catalogue cycle filled the `realtime` lane's per-scope slots ahead of a buyer's order, *and* the catalogue was throttled to a cap deliberately sized small for waited-on work. The measurement made the second half unmistakable: raising the connection's own rate limit from 60 to 300 req/min moved shop traffic from 50 to only 63 req/min, because the limiter was never the ceiling - the lane cap was. A sibling instance had the same flavour with a different field (#2609): `inventory.propagateToMarketplaces` was enqueued with a synthetic all-zero connection id, so every stock write in the installation shared one scope and a `fan-out` per-scope cap of 1 serialised all of them - measured on the demo stack as a queue growing ~145 jobs/h faster than it drained. Right lane, meaningless scope.

**Rule**: before registering a job type in a lane, ask *who is waiting on this instance* - and if the answer differs by trigger, **split the type** (`…syncFromSweep` beside `…syncByExternalId`, both registered against the same handler instance) rather than picking the lane that hurts less. Keep both types registered across a deploy so a child queued under the old name still runs and the boot coverage assertion still holds. Separately, check that the job's `scope` names something real: a synthetic or constant scope id silently turns a per-scope cap into a global one, and the symptom (a queue that grows under ordinary operation) looks like a capacity problem rather than a configuration one. And do not reach for a new lane - ADR-050 decision 1 makes a fifth lane a reversal gate, not a tuning step.
