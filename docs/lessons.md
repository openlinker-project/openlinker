...
**Sweep the whole method, not the one column you came for**: #2101 fixed `fulfillmentState` and left its exclusion comment sitting directly *below* two more assignments with the identical defect (`syncStatus`, `syncAttempts`), which then needed #2140. When you exclude a column here, re-read every remaining assignment in `toOrm` and ask which out-of-band writer owns it. Two follow-on traps that fix surfaced: (1) an excluded **array** column needs a `?? []` guard where `toDomain` reads it, because the update path has no `RETURNING` clause and hands the property straight back `undefined` - `null`able scalars were already guarded, so #2101 never hit it; (2) check the DB default before assuming a migration is needed - a `NOT NULL` column is fine to omit when the migration gave it a `DEFAULT` (TypeORM emits `DEFAULT` for an `undefined` column on Postgres), and it is the ORM decorator, not the schema, that needs the missing `default` to end the drift.
