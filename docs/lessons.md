...
**Rule**: express a destination's accepted markup as a `parent -> allowed children` content model with explicit rewrites, never as a flat allowlist, and never as a regex private to one adapter. The model, per destination: top level `h1 h2 p ul ol`; inside `<p>` only `b`; inside `<ul>`/`<ol>` only `li`; inside `<li>` `b` and `p`; inside `<h1>`/`<h2>` **nothing**. `strong em i u br div span` are rejected unconditionally, and `<br>` needs *converting* to a paragraph break rather than stripping, because Allegro's own answer is "use `<p></p>`". When you widen an allowlist, ask what evidence you have for the new tag; on Allegro the only evidence available is a rejection message, so a spec must pin the exact set and widening it has to be a deliberate test change.
