...
Some configuration is **not** seeded automatically and must be applied once per
deployment before the corresponding flow works end to end. This section is the
**index** of those manual standup steps — each entry links to the doc that owns
the field-level detail rather than restating it. Most of these fail *silently* — the
flow simply never runs, with no error at boot — which is exactly why they need a
checklist rather than being discovered from a stack trace.

### 8.1 Worker scheduler flags (most default OFF)

Several poll-based flows are gated by env flags read by the **worker**, and the
gate is checked *before* any per-connection capability check — so an unset flag
disables the flow for every connection at once, with no error anywhere. Set them
explicitly on the demo worker rather than relying on defaults:
`OL_ERLI_ORDERS_POLL_SCHEDULER_ENABLED`,
`OL_ERLI_OFFER_STATUS_SYNC_SCHEDULER_ENABLED`,
`OL_ERLI_DISPATCH_WRITEBACK_ENABLED`,
`OL_WOOCOMMERCE_POLL_SCHEDULER_ENABLED` (all **OFF** by default) and
`OL_DPD_SHIPMENT_STATUS_SYNC_SCHEDULER_ENABLED` (**ON** by default). Allegro and
PrestaShop schedulers need no demo-specific flag. Full table with cadences and
demo values: [walkthrough → Scheduler flags](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/README.md#scheduler-flags-demo-worker-env).

### 8.2 External prerequisites (obtained outside OpenLinker)

Nothing here can be seeded — each needs an account or token issued by a third
party, so budget lead time before a standup:

- **DPD sandbox credentials** — login/password + payer FID, issued by DPD.
  ([DPD walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/06-dpd.md))
- **InPost ShipX** — `organizationId` provisioned by InPost + a portal-generated
  Bearer `apiToken`. InPost also has **no self-service webhook provisioning**:
  the connection's **Actions** tab generates an email template you send to
  `integration@inpost.pl` with the callback endpoint.
  ([InPost walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/05-inpost.md))
- **KSeF test token** — generated by hand on `https://ap-test.ksef.mf.gov.pl`;
  the seller NIP also has to be filled on the connection config.
  ([KSeF walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/07-ksef.md))
- **Allegro app registration** — app + OAuth redirect URI registered in Allegro's
  developer portal, then the seller defaults (ship-from location, Responsible
  Producer, safety information) filled on the connection or offer creation fails
  with `SELLER_DEFAULTS_NOT_CONFIGURED`.
  ([one-command demo guide → Allegro](./one-command-demo-setup-guide.md#6-wire-the-allegro-connection-optional))

### 8.3 Connection-level configuration

Apply in this order — the routing step can only point at connections that
already exist:

1. **Carrier connections (InPost / DPD)** - create one connection per carrier by
   hand with the credentials from § 8.2. Get DPD's **sender postal code** right:
   it must be real, deliverable, and consistent with the sender city, or DPD
   rejects *every* shipment. Since #1781 OpenLinker validates deliverability at
   setup and surfaces DPD's real rejection reason, so this now fails at connection
   creation rather than silently at the first label — but it still has to be
   entered correctly by hand.
   ([DPD walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/06-dpd.md))

2. **Erli fulfillment routing** - an Erli source connection ingests orders out of
   ...
   them on the connection's **Mappings -> Fulfillment** tab, or via
...
3. **Erli Allegro-category browsing** (optional but strongly recommended) - tick
   the checkbox on the Erli connection, choose *reuse credentials from an existing
   Allegro connection*, then click **only "Save credentials"**. Clicking the page's
   bottom **"Save changes"** clobbers the flag from a stale snapshot, silently
   reverting it — so this needs re-doing on every fresh deploy until that's fixed.
   Without it, Erli offer creation falls back to a raw free-text Allegro-category-ID
   field instead of the real tree picker.
   ([Erli walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/04-erli.md))

The per-integration [Phase-1 manual walkthrough](./manual-testing/01-presta-woo-allegro-erli-inpost-dpd-ksef/README.md)
is the fuller click-by-click record of a standup run — including how to trigger a
scheduled sync job by hand instead of waiting out its cadence, and how to keep
`cloudflared` tunnel URLs in sync with connection config after a tunnel restart.
Connection-wiring specifics for PrestaShop, WooCommerce and Allegro (internal vs
public URLs, webservice key generation) live in the
[one-command demo setup guide](./one-command-demo-setup-guide.md). When you hit a
new manual standup action, add it here rather than only in the walkthrough.

**Deliberately not on this list**: activating the Poland (PL) country in
PrestaShop (#1446). PrestaShop ships PL inactive, which breaks order sync for a
PL buyer address — but this is now seeded automatically at container boot by
`docker/prestashop/post-install/25-activate-country-pl.sh`, so it needs no manual
step. Listed here only so it isn't re-added as one.
