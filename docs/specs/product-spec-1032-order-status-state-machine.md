...
1. **The end-state is sharper than "guarded canonical axis".** The canonical lifecycle is a *derived
   projection over a per-axis fact ledger*, never a stored contested scalar — [ADR-039](../architecture/adrs/039-order-lifecycle-derived-from-fact-ledger.md).
...
2. **A changeset model was missing from the original spec.** Every OL order mutation is a proposal to
   a remote authority that may decline it — [ADR-040](../architecture/adrs/040-order-changeset-proposed-then-confirmed.md).
