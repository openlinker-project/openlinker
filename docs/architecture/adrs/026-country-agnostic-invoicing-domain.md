...
### Annex (2026-08-13, #2009) - where a line's tax rate comes from

Decision 1 gave every line a neutral `taxRate` string but never said who supplies its value. In practice nobody did: the order-to-command mapper emits `taxRate: ''` (`libs/core/src/invoicing/application/mappers/order-to-issue-invoice-command.mapper.ts:228`, `:249`) and each provider adapter substitutes its own guess. This annex records the rule decided on 13 Aug 2026, when a drafted tax-calculation ADR (a `TaxCalculationPort` engine-facing fourth capability) was **withdrawn in full as overengineering**. It records the rule only; the contract and code work is #2054.
...
- **A tax engine (Avalara / TaxJar / Vertex) is an integration inside the master or the sales channel - a shop plugin - never a port in OpenLinker.** Rejected alternative, and why: the rate is a function of *(product, delivery country)*, and all three inputs an engine needs (product tax category, delivery address, seller nexus) are held by the shop, not by an aggregator. [ADR-014](./014-source-authoritative-order-pricing.md) already establishes the governing principle for the sibling value on the same record - the buyer-paid price is authoritative and is never recomputed; the rate is the same class of data.
- **The carrier is a code, not a number.** `0`, `zw`, reverse charge and intra-EU 0% are four different things on a document and all look like zero. `InvoiceLine.taxRate: string` already exists (`libs/core/src/invoicing/domain/types/invoicing.types.ts:280`) and is part of the frozen `IssuedLineSnapshot` (`:329-336`), so the rule itself implies no migration.
- **A missing rate is a blocking business failure** with an operator-facing reason - never a silent `0`, never a silent `23`.
- **Where a channel also reports a rate, it is a cross-check and not an authority.** Today only Erli does (`libs/integrations/erli/src/infrastructure/adapters/erli-order.types.ts:79`); Allegro reports gross only and does not decompose tax (`allegro-order-source.adapter.ts` - `tax: 0`, `taxTreatment: 'inclusive'`). A mismatch against the master puts the order into a **tax-rate conflict** state that **blocks both invoice issue and fiscalisation** until an operator decides; the decision is persisted with actor and timestamp, and `AutoIssueTriggerService` must not fire on an unresolved conflict.
- **Analytics requirement, same decision:** the order snapshot stores **net and gross per line** alongside the rate as persisted values, not values derived on read. Today a line carries a single `IncomingOrderItem.price` with no tax field at all, and the only gross/net signal is `taxTreatment` - one optional flag for the **whole order** (`libs/core/src/orders/domain/types/order.types.ts:264`), which cannot describe a mixed-rate basket.
