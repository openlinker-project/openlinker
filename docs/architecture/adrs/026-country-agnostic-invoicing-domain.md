...
**Amendment (2026-08-13, #2009) - where a line's tax rate comes from.** Decision 1 gave every line a neutral `taxRate` string but never said who supplies its value. In practice nobody did: the order-to-command mapper emits `taxRate: ''` (`libs/core/src/invoicing/application/mappers/order-to-issue-invoice-command.mapper.ts:228`, `:249`) and each provider adapter substitutes its own guess. This records the rule decided on 13 Aug 2026, when a drafted tax-calculation ADR (an engine-facing `TaxCalculationPort`) was **withdrawn in full as overengineering**. It records the rule only; the contract and code work is #2054.
...
**Supersession required, conducted elsewhere.** ADR-014 rejected a per-line tax rate on `OrderItem` under "Alternatives considered". Carrying this rule therefore requires superseding that rejection; the argument and ADR-014's own supersession note are **#2054's**, deliberately not restated here so the two documents cannot drift apart. Fiscalisation carries the same rule for its own reason (a fiscal registration transmits amounts it must not recompute) - see [ADR-040](./040-fiscalisation-capability.md).
