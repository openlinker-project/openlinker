...
**Amendment (2026-08-13, #2009) - where a line's tax rate comes from. Status: PROPOSED, pending the ADR-014 reversal in #2058.** Decision 1 gave every line a neutral `taxRate` string but never said who supplies its value. In practice nobody did: the order-to-command mapper emits `taxRate: ''` (`libs/core/src/invoicing/application/mappers/order-to-issue-invoice-command.mapper.ts:228`, `:249`) and each provider adapter substitutes its own guess. This records the rule reached on 13 Aug 2026, when a drafted tax-calculation ADR (an engine-facing `TaxCalculationPort`) was **withdrawn in full as overengineering**. Unlike the amendments above it does **not** merely refine a deferred detail (see the note at the top of this section): adopting it reverses a rejection ADR-014 still records as live, so **until #2058 is accepted the bullets below are the proposed rule, not current architecture**. It records the rule only; the contract and code work is #2054.

- **The VAT rate arrives from the ProductMaster, together with the product.** OpenLinker never computes it, never infers it, never substitutes a default.
- **A tax engine (Avalara / TaxJar / Vertex) is an integration inside the master or the sales channel - a shop plugin - never a port in OpenLinker.** Rejected alternative, and why: the rate is a function of *(product tax category, delivery address, seller nexus)*, and the product tax category is the shop's alone. (The other two are not a reason to place the engine there: OL itself ingests the delivery address from the marketplace and pushes it into the shop, and seller registrations are connection-level config OL already holds.) [ADR-014](./014-source-authoritative-order-pricing.md) already establishes the governing principle for the sibling value on the same record - the buyer-paid price is authoritative and is never recomputed; the rate is the same class of data, and that argument carries the decision on its own.
- **The tax rate is a code, not a number.** On a Polish invoice the markers are `23/8/5/0`, `zw` (zwolnione), `np` (nie podlega) and `oo` (odwrotne obciążenie) - all distinct, and three of them look like zero; a stawka 0% used for WDT is a *use* of `0`, not a further code. On a Polish **paragon** the rates are carried as letters **A-G**, which is a second, fiscal-side reason the value must be a string. `InvoiceLine.taxRate: string` already exists (`libs/core/src/invoicing/domain/types/invoicing.types.ts:280`) and is part of the frozen `IssuedLineSnapshot` (`:329-336`), so **on the invoicing side** the rule implies no migration. The product-side field, the conflict state and the per-line net/gross columns below are new persisted state, and are #2054's.
- **A missing rate is a blocking business failure** with an operator-facing reason - never a silent `0`, never a silent `23`.
- **Where a channel also reports a rate, it is a cross-check and not an authority.** Today only Erli reports a per-line **rate** (`libs/integrations/erli/src/infrastructure/adapters/erli-order.types.ts:79`); Allegro reports gross only and does not decompose tax (`allegro-order-source.adapter.ts` - `tax: 0`, `taxTreatment: 'inclusive'`), while WooCommerce reports an order-level tax amount but no per-line rate. A mismatch against the master puts the order into a **tax-rate conflict** state. That state blocks both invoice issue and fiscalisation until an operator decides. The decision is persisted with actor and timestamp. The flag is carried on the `Order` projection `AutoIssueTriggerService` already receives, so that service can refuse to fire on an unresolved conflict without injecting an orders token - preserving its documented one-way edge (F3).
- **Analytics requirement, same decision:** the order snapshot stores **net and gross per line** alongside the rate as persisted values, not values derived on read. Today a line carries a single `price` with no tax field at all (`libs/core/src/orders/domain/types/incoming-order.types.ts:131`, `:146`), and the only gross/net signal is `taxTreatment` - one optional flag for the **whole order** (`libs/core/src/orders/domain/types/order.types.ts:283`, mirrored on `IncomingOrderTotals` at `incoming-order.types.ts:190`), which cannot describe a mixed-rate basket.

**Supersession required, still open.** ADR-014 rejected a per-line tax rate on `OrderItem` under "Alternatives considered", and **that rejection stands until it is reversed**. The reversal is proposed in **PR #2058** (step 1 of #2054) as an amendment on ADR-014 itself; it is a draft, open, and may be refused. The argument and ADR-014's own supersession note are **#2058's**, deliberately not restated here so the two documents cannot drift apart - and neither document is normative until #2058 lands, so this annex must not be read as settling what #2058 is asking. Fiscalisation carries the same rule for its own reason (a fiscal registration transmits amounts it must not recompute) and under the same caveat - see [ADR-040](./040-fiscalisation-capability.md) decision 8.
