...
**Pros:**
- No new HTTP endpoints; the existing generic capability-resolution machinery (`CategoriesCacheService`, `listings.controller.ts`) works unchanged for an Erli `connectionId`.
- Bulk wizard and single-offer wizard can both gate on the same `supportedCapabilities` signal — removes a latent inconsistency where only the bulk wizard has this check today.
