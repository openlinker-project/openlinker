...
**2. The in-flight lease becomes readable.** A concurrent attempt raises a 409. That is the correct answer to a write and useless to a reader: the surface cannot distinguish *someone else is registering this right now* - reassuring, requiring no action - from an error. The lease becomes a readable fact on the per-order projection ([ADR-065](./065-sales-document-read-surface.md), which lands with #2501), so a surface can state it without attempting a registration. Visibility only: the lease semantics, the exactly-once guarantee and the 409 are unchanged.
...
**4. Manual registration becomes asynchronous, and only then may any surface say the work survives navigation.** `POST /fiscal-registrations` calls the blocking adapter inline while the auto-issue gate already enqueues `fiscalization.register`. Two paths to one act, one of which dies with the tab: the *record* survives, the operator's view of it does not. The manual path moves onto the same job with the same idempotency key, plus a pollable read. Until that ships, no surface may claim background continuation - which is why the mockup (`docs/plans/mockups/sales-document-routing.html`, the `receipt-registering` scenario) deliberately tells the operator to keep the page open and names what happens if they do not: *"Keep this page open until it answers. If you close it, the request stops and nobody will know whether a receipt was created. The order will read Unconfirmed."* The same page lists *"Safe to leave this page"* among the lines it cut as untrue.
