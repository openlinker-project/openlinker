...
## Amendment (#2603 / #2604 / #2614 / #2652) - the shop-side outbox, and the one rule the three fixes share

This ADR's decisions govern OL's own durability spine. The PrestaShop module runs its own outbox on the far side of that spine - `openlinker_webhook_outbox`, a MySQL table the module writes in the same request as the shop-side change and a cron drains over HTTP - and epic #2590 fixed it in three places. The fixes belong here because the outbox is the producer end of the webhook path decision 1 shipped, and because they are one rule seen three times:

> **Reducing retry pressure must never become dropping work.** Every mechanism that makes the outbox cheaper - dedup, retention, backoff, a run budget - is a mechanism that can silently discard an event, and each one has to be built so that the discard is impossible rather than unlikely.

**Coalescing moved off a clock window onto a releasable key (#2603).** The event id used to embed a *rounded time window*, so a burst of changes to one product collapsed onto one id - and because the unique index covered already-delivered rows too, a second change inside the window was silently dropped. The id is now unique per row (it carries `microtime` plus entropy), and coalescing is expressed by a separate **nullable** `dedup_key` with its own unique index, seeded from `(provider, connectionId, eventType, objectType, externalId)` and **no time component at all**. The key is **released to `NULL` at CLAIM time**, not at delivery: the HTTP POST happens between claim and mark, so releasing at claim means a change arriving in that gap gets its own row. The cost is a possible duplicate row when a claimed batch is requeued - harmless, since each row carries its own event id and the OL-side gate is idempotent - and that is the right side to err on. Nullability is what makes the migration backfill-free (MySQL treats NULLs as distinct), and every write is guarded by a cached column probe so a shop that has not run the upgrade cannot have a row stranded by a missing column.

**Retention deletes only terminal rows, and never gates intake (#2604).** The delete builder takes the status as a parameter and **throws** unless it is exactly `delivered` or `failed` - a positive two-value whitelist, not an exclusion list, so a future status cannot fall through into a prune. Age-based deletes run first; a row cap is enforced only by deleting more *terminal* history, oldest delivered then oldest failed. When the table is still over its cap and there is no prunable history left, the pass **reports `backlog_over_cap`** and logs that the excess is undelivered events - it does not delete a `pending` row and it does not refuse a new one. That is the rule stated exactly: an over-cap table is a delivery problem to surface, never a reason to discard the events that prove it. The pass is additionally budgeted and rewinds its own last-run stamp when it spends the whole budget, so a large backlog drains across passes instead of monopolising one.

**Backoff gained an endpoint-level failure streak, jittered downward only (#2614).** Per-row exponential backoff paces one row and does nothing about the case that actually matters - the OL endpoint being down, where every row fails on its own schedule and the shop hammers a dead host. A per-endpoint streak, counted **once per cron pass** however many rows failed and reset on the first success, produces a second delay; the row waits the longer of the two. The endpoint delay carries a much lower ceiling than the row delay **because it is also the recovery probe interval** - a streak that backed off to hours would leave a recovered endpoint undiscovered for hours. Jitter is applied **downward only**, over `[0.5x, 1x]` of the computed delay with a floor of one second: jitter exists to break the thundering herd, and a scheme that could shorten a delay toward zero, or lengthen it past the ceiling, would defeat the backoff it decorates.

**A delivery run is bounded by wall clock, and a reclaim cannot steal a live lease (#2652).** The pass had no time bound of any kind, so a full batch of slow deliveries could exceed a shared host's process-kill limit and be killed mid-run - which is the same class of loss, arriving through the process manager. The loop now checks a budget **before** each delivery, using the worst-case delivery time rather than the average, and requeues everything it did not reach with an explicit reason. Two guards make the bound safe rather than merely present: the first delivery of a pass is always allowed (a budget must never mean "deliver nothing"), and the stale-lease threshold that reclaims an abandoned row is **derived from the budget** (budget + worst-case delivery + a safety margin) rather than configured independently, so a live run's lease can never expire underneath it. The reclaim additionally excludes the calling run's own rows by owner - previously the manual reclaim had neither an age nor an owner predicate, and could take a lease from a run in progress.
...
- Related issues: #2162, #2163, #2164, #2165, #2280, #1135, #1134, #1396, #2603, #2604, #2614, #2652 (the shop-side outbox amendment)
