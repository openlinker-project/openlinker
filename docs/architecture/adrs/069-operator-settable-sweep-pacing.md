...
- A platform's own wall stays where the request is built rather than being smuggled into this table as our opinion: an adapter clamps to its own `per_page` and **warns when the clamp bites**, because a silent narrowing is the same defect one layer down.
