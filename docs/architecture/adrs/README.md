...
| [ADR-052](./052-per-line-tax-rate-resolution-and-provenance.md) | Per-line tax-rate resolution, provenance and rounding ownership - master first then channel, three answer states, projection at sync, adapter-owned rounding | Proposed | 2026-08-21 |
...
> *Reserved numbers: **039** is claimed by #2014 (order analytics read model — already referenced by name from `docs/plans/implementation-plan-order-cancellation-record-state.md` on `main`). **040 / 042 / 043 / 044 / 045** are no longer reserved — #2050, #2056 and #2066 carry their rows above. Each open PR adds its own row when it merges, so the gap at 039 is deliberate, not a lost number. **049-051 are claimed by the #2162 async-work-layer epic**, allocated up front so its remaining ADR branches cannot race each other for a number: **049** = #2165 (durability spine and the domain-event contract), **050** = #2167 (workload isolation / concurrency lanes), **051** = #2168 (worker topology / roles). The epic's fourth, **048** (#2166, incremental catalog replication), is no longer reserved - it carries its own row above. Note that #2162 and its sub-issues were written against 046-049, which 046 and 047 had already been taken from - read the allocation here, not the issue titles. **052 is taken by #2246 (per-line tax-rate resolution) - it carries its own row above. Allocate 053 for the next new ADR.** A number stays reserved until its PR merges or closes; if one is abandoned, reuse the number rather than leaving it dark. Nothing currently guards ADR numbering mechanically — tracked as #2082.*
