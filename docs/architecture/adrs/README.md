...
1. Copy [`template.md`](./template.md) to `NNN-kebab-case-title.md` where `NNN` is the next free 3-digit number — pick one higher than the highest number in the index below. `scripts/check-adr-numbers.mjs` (run from `pnpm lint`) fails the build if it's already taken, not just by another file in this tree but by anything already on `origin/main` — so a collision with a sibling PR is caught at merge time rather than producing a silent duplicate.
...
| [ADR-072](./072-buyer-tax-identity-and-dual-role-document-connections.md) | Buyer tax identity, the fiscalization/invoicing split, and dual-role connections | Accepted | 2026-09-11 |
...
> *Reserved numbers: this was previously a hand-maintained log of in-flight number claims, kept to steer authors away from colliding with an open PR - it drifted from reality at least three times in two days (#2050 vs #2056 on `040`; #2066 colliding with three branches on `039`/`040`/`041` at once) because a note can't see a PR opened after it was written. `scripts/check-adr-numbers.mjs` (#2082) now enforces uniqueness mechanically at `pnpm lint` time, against both the local tree and `origin/main`, so this note is no longer needed to prevent a collision — pick the next free number and let the guard catch it if someone else already took it. Historical note: ADR-057 supersedes ADR-017 and ADR-059 supersedes ADR-043 **on acceptance, not on filing** - both older ADRs keep their status until then.*
