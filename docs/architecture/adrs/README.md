...
> *Reserved numbers: **039** is claimed by #2014 (order analytics read model — already referenced by name from `docs/plans/implementation-plan-order-cancellation-record-state.md` on `main`), **042** by #2056 (fiscalisation capability), and **043 / 044 / 045** by #2066 (OMS research record). **040** is no longer reserved — #2050 carries its row above. Each open PR adds its own row when it merges, so the gaps are deliberate, not lost numbers. A number stays reserved until its PR merges or closes; if one is abandoned, reuse the number rather than leaving it dark. Nothing currently guards ADR numbering mechanically — tracked as #2082.*
