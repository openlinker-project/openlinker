...
> *Reserved numbers: **039** is claimed by #2014 (order analytics read model — already referenced by name from `docs/plans/implementation-plan-order-cancellation-record-state.md` on `main`). **040 / 042 / 043 / 044 / 045** are no longer reserved — #2050, #2056 and #2066 carry their rows above. Each open PR adds its own row when it merges, so the gap at 039 is deliberate, not a lost number. **Allocate 046 for the next new ADR.** A number stays reserved until its PR merges or closes; if one is abandoned, reuse the number rather than leaving it dark. Nothing currently guards ADR numbering mechanically — tracked as #2082.*
