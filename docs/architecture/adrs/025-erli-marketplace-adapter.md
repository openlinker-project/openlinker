...
3. **Allegro-ID taxonomy reuse, with a shop-native fallback (#1096).** Erli offers prefer the product's already-resolved **Allegro** category id, tagged `source:"allegro"` (the concrete field names are pinned by the implementing PR, #984+). When no Allegro category is resolved, the adapter falls back to the master shop's own categories tagged `source:"shop"` (Erli's `externalCategories` accepts `source ∈ {shop, allegro, marketplace}` with a required `breadcrumb`, so an operator who never connected Allegro still lists — categorised by their own shop tree). When neither is present the offer lists **uncategorised** (Erli makes the category optional) rather than failing — this supersedes the original v1 stance of skipping such products with a hard error. OL still builds no Erli-native taxonomy *authoring* path; it only reuses ids it already holds (Allegro-resolved or shop-source). Parameter reuse (`externalAttributes`) remains Allegro-id-only in v1.
4. **Adapter-level stock/ownership invariants.** The adapter owns two Erli-specific compensations: (a) on order cancellation OL issues a stock-restore PATCH (because Erli won't) — **wired by #1146**: the marketplace-neutral `OfferStockRestorer` capability + the core `OrderIngestionService` cancellation-observe hook (which enqueues `marketplace.offer.stockRestore` → core `OfferStockRestoreService` → `restoreStockOnCancellation(targets)`). Core resolves the absolute master-inventory target per offer and passes plain `OfferStockRestoreTarget[]`; the adapter just absolute-sets each via `updateOfferQuantity`; and (b) on the field-update path (`updateOfferFields` — `price`/`name`/`description`) the adapter reads the live resource and excludes any field the seller marked `frozen`, so OL never overwrites a manual edit. Frozen `stock` is **not** honored on the hot quantity path (`updateOfferQuantity`) in v1 — that path skips the read-before-write GET for per-tick performance; honoring it without a per-tick GET (via a frozen flag cached during reconciliation) is **deferred to #1066**.
...
- Allegro-ID reuse couples Erli *category-on-Allegro* quality to the operator's Allegro taxonomy hygiene, but products without Allegro data still list (#1096): they fall back to `source:"shop"` categories from the master, or list uncategorised. Parameter (`externalAttributes`) reuse stays Allegro-id-only, so non-Allegro products list without marketplace parameters in v1.
- Frozen-field exclusion means OL silently skips **content** fields (`price`/`name`/`description`) the seller edited manually (no conflict UI in v1 — spec #978 §6). Frozen `stock` on the quantity path is a v1 follow-up (#1066); the cancel-restore PATCH is now wired via the `OfferStockRestorer` capability + the core cancellation-observe hook (#1146).
...
- Setup guide: [libs/integrations/erli/docs/setup-guide.md](../../../libs/integrations/erli/docs/setup-guide.md)
- Runbook: [libs/integrations/erli/docs/runbook.md](../../../libs/integrations/erli/docs/runbook.md)
