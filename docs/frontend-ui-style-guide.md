...
- **Every leading control needs `vertical-align: top` — and so does the identity cell, if anything else on the row is taller.** `.data-table td` is `middle`, so at ~60 px a 24 px expander or a 13 px checkbox centres opposite the gap between the two identity lines rather than the identity it belongs to. Alignment only, never a `padding-top` nudge: the cell already inherits the table's vertical padding, so top-aligning lands the control on line 1 at every row height, while a padding tuned for one height misaligns the other tables.
  ...
  Pick by asking what sets the row height. If it is the identity cell, align the leading control. If it is another column, align the whole row. **If there is no leading control, do nothing** — Customers is that case, and reaching "nothing to do" by elimination is correct, not an oversight. Note the reason is the missing control, not the row height: before #2093 Customers was single-line throughout, and adopting `ConnectionCell` made its Source column the tallest thing on the row without changing the verdict. Its remaining columns are single-line and stay `middle`, which is what every other table's single-line neighbours already do; only a *control* misreads centring, because it points at one line rather than being one.
