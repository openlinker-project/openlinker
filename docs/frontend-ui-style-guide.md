...
- **A shared cell's own cap is a budget for a specific adornment; re-measure it if you pass a different one.** `.connection-cell__body`'s 220 px was measured for an adornment that is absent or a 14 px `ConnectionDot`. Products passes a `.channel-pill` — mono text, ~95 px for `WooCommerce` — which leaves the connection *name* ~120 px, enough for two same-platform connections to ellipsise to the same string and stop discriminating on the axis the line exists for. Widen per page (`.products-table .connection-cell__body`), positively, the same way a frozen-pane narrowing is scoped.
