...
| 9 | [`09-subiekt-gt.md`](./09-subiekt-gt.md) | ERP: catalogue, orders, documents, stock | ⚠️ Fourth round on PR #3365 — **code-complete, deployed, proven in halves**. Bridge and images rebuilt and verified to carry the fixes; `subiekt` project 7/9 pass. The two failures are a Hyper-V firewall and the stand's two catalogues not overlapping, which OpenLinker refuses with the right sentence. The end-to-end chain needs a publish-only shop mastered by Subiekt; three routes to create one are closed for real reasons and a fourth was declined because it would have made the fixture lie — Part K |
