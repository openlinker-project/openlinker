...
1. **Unique**: no two files across `apps/api/src/migrations/` AND every plugin migration directory listed in `scripts/plugin-migration-dirs.json` (#599) share a 13-digit prefix. Uniqueness is enforced across the *union*, so Allegro + a hypothetical Shopify plugin can't both pick the same prefix.

   **What a collision actually does — corrected, read against the installed package rather than against this doc.** An earlier revision of this rule said a duplicate timestamp "can leave one `up()` body silently unapplied while both class names still appear in the `migrations` table". That is not TypeORM's behaviour and the claim had propagated into three commit messages before a reviewer opened the file. In `typeorm@0.3.17`, pending is decided by **class name** — `MigrationExecutor.js:79`, `!executedMigrations.find((e) => e.name === migration.name)` — so two files sharing a prefix but declaring differently-named classes **both run**. What is genuinely undefined is their relative **order**: `:425` is `migrations.sort((a, b) => a.timestamp - b.timestamp)` with no tie-break, so a collision between two migrations where one depends on the other can apply them the wrong way round, and which way is not reproducible between machines.

   So the real cost of a duplicate is (a) a non-deterministic apply order, and (b) a **merge gate**: `check-migration-timestamps.mjs` refuses a duplicate within one tree, so whichever branch merges second fails `pnpm lint`. That is a loud failure, not a silent one — the rule stands, the reason changed. Historical note: [#374](https://github.com/openlinker-project/openlinker/issues/374) is the issue this rule was written under; its own description predates this reading.
