...
All three rules are enforced by `scripts/check-migration-timestamps.mjs`, chained into the root `check:invariants` command and therefore into every `pnpm lint` run (including pre-commit). A collision fails `pnpm lint` immediately; the ordering check compares the working tree against `origin/main` via `git ls-tree` and is skipped with a notice when that ref is unavailable (CI always has it).
