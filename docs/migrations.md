...
All three rules are enforced by `scripts/check-migration-timestamps.mjs`, chained into the root `check:invariants` command and therefore into every `pnpm lint` run (including pre-commit). A collision fails `pnpm lint` immediately; the ordering check compares the working tree against `origin/main` via `git ls-tree`. When that ref is unavailable the check is skipped with a notice **locally**, but in CI (`CI=true`) a missing ref is a **hard lint failure** — the `lint` workflow job fetches `origin/main` after checkout (`actions/checkout@v4` otherwise shallow-fetches only the triggering ref), so the ordering invariant runs on every PR build. `push: [main]` builds pass it vacuously (the migration is already in the baseline) — the guard is a pre-merge gate (#1020).
