...
split as Infakt - it hands the document to the national e-invoicing hub on the
seller's behalf and reads back the clearance, never holding the authority
session itself, and reaches the vendor's sibling `eCorrectiveInvoice` document
kind for a correction rather than a per-line delta on the original. (See the
