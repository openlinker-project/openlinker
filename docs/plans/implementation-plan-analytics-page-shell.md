...
1. **Pure helpers — `features/analytics/lib/ingestion-trust.lib.ts`**
   ...
   - Action: One pure, unit-testable function — no date-range parameter (per Decision 4, the range-gated version is deferred):
     - `shouldShowDegradationBanner(entries: ConnectionIngestionTrust[]): ConnectionIngestionTrust[]` — filters entries where `status` is `'stalled' | 'disconnected'`. Returns the list (usually 0 or 1) so the banner can name every offender; caller renders one `Alert` per entry, or none.
...
2. **`features/analytics/components/analytics-degradation-banner.tsx`**
   ...
   - Action: Renders zero or more `Alert tone="error"` blocks, one per entry returned by `shouldShowDegradationBanner`, with copy `"{connectionName} has not ingested since {lastPollAt formatted}"` + description "This is an ingestion gap, not a drop in sales." + a `Button tone="secondary"` sized `sm` labelled "View sync" linking to `/cursors?connectionId={id}` (existing cursors page — closest existing "sync detail" destination; no new page needed). Renders `null` when the input list is empty.
...
1. **`features/analytics/lib/date-range.lib.ts`**
   ...
   - Action: Pure helpers, unit-tested in isolation from any component:
     ...
     - `toUtcRangeInstants(from: string, to: string): { fromIso: string; toIso: string }` — `from → T00:00:00.000Z`, `to → T23:59:59.999Z`, mirroring `orders-list-page.tsx:268-273`.
   - Acceptance: unit tests — preset math for a fixed `today`, boundary/leap-year case, `derivePreset` round-trips a preset-computed range back to that preset, an arbitrary custom range returns `'custom'`.
   - Dependencies: none.
...
- **Coverage-row semantic risk**: shipping "Connected since" where the eventual design wants "Data from" could read as a UI regression once #1985 lands. *Mitigation*: Decision 3 documents the exact swap needed and why it isn't done now; **filed as [#2083](https://github.com/openlinker-project/openlinker/issues/2083)**, blocked by #1985 — not a loose end.
