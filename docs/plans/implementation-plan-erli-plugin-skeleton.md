...
- **Erli Shop API**: REST over HTTPS, GET/POST/PATCH only, static API-key bearer auth, async writes (HTTP 202 + ~20-min cache lag), inbox-based order feed, no-retry/5 s webhooks. Docs: https://erli.pl/svc/shop-api/doc/. None of this is exercised by the skeleton — it shapes ADR-022 and the capability roadmap (#984/#993).
