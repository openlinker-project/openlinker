...
### Out of Scope
- **Update (post-review)**: `apps/api/src/mappings/http/mapping-options.controller.ts`'s `source/categories` / `source/categories/:categoryId/path` routes weren't named in the issue's file list and declared **no** 422 at all. They resolve through the identical `resolveScope` → `resolveDestination` → `tryGetAdapter` chain as the two listed controllers, so leaving them undecorated would have been an inconsistency the reviewer would reasonably flag — `@ApiResponse` 404/409 decorators were added there too, beyond #2146's literal scope but consistent with its intent.
...
- **Scope creep, resolved**: `mapping-options.controller.ts`'s two previously-undecorated routes start returning 404/409 as a side effect of this fix. Rather than leave them undocumented, `@ApiResponse` decorators were added there too — beyond the issue's literal file list, but consistent with restoring accurate docs across every route that shares the fixed resolution chain.
