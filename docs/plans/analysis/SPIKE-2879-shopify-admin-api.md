...
> ⚠️ **Work in progress.**
> This document consolidates everything verified so far about the Shopify Admin GraphQL API into
> **one** reference organized by capability area (connection, catalog, taxonomy, orders, fulfillment,
> tax/currency, returns, platform limits) rather than by when it was tested. Facts marked with a live
> transcript are **verified** against a real development store (`{shop-domain}.myshopify.com`,
> custom-distribution app `Test-OLL`, API version `2026-07`); everything else is desk research carried
> over from issue #2879 and marked `DESK` — treat it as a hypothesis to confirm, not a fact to build
> on. This file is updated in place as more stories are verified; it is not yet complete enough to
> close the parent issue.

Sources of record for the verified sections: live GraphQL Admin API calls via `curl`, transcripts
captured in `## Live transcripts`. **Not every "Live"-marked row has a captured transcript** — where
the Evidence column reads "Live — observed, no transcript retained" the call was made live but its
request/response was not preserved in this document; treat those rows as a live observation the
author stands behind, not as a reproducible artifact. Every row whose Evidence column instead says
"Live transcript below" is backed by an actual fenced code block under `## Live transcripts`. Desk
research baseline: issue #2879 itself (which cites shopify.dev pages fetched 2026-09-04) and
`docs/plans/analysis/ANALYSIS-new-marketplace-integrations-story-catalogue.md` (#2883).

## Environment

Dev store `{shop-domain}.myshopify.com` (Partner org, plan reported as `"Advanced App Development"`),
custom-distribution app `Test-OLL` (`client_id = REDACTED`), Admin API version `2026-07`. Access
token obtained via full OAuth authorization-code grant
(`https://{shop-domain}.myshopify.com/admin/oauth/authorize` →
`https://{shop-domain}.myshopify.com/admin/oauth/access_token`); token prefix returned is `shpua_`,
not the classically-documented `shpat_` — **but it is stable across every repeated OAuth exchange for
the same app+shop+scope, including after later scope additions**, which is the behavioural signature
of an *offline* token, not a per-session online one. Flagged as E-C3 below for confirmation against
official docs (name of this prefix is not found in the desk-research pass). Cost budget across every
verification pass stayed comfortably inside the per-app-per-store allowance (`maximumAvailable:
4000`, `restoreRate: 200` — see E-C4); no throttling was ever observed on this store.

Verification proceeded incrementally as scope gaps were discovered — three scopes had to be added
after the initial OAuth grant, each via a fresh grant against the **same** app+shop, and the token
value did not change (see `## Open risks` item 5 and `## Recommendation` item 1 for the reconciled,
final scope list). Later passes deepened or corrected earlier findings in place; where a later fact
supersedes an earlier one, the earlier claim is not repeated separately — the table below states only
the current, reconciled understanding, with the correction noted inline where it matters (e.g.
E-R9/E-R10 correct E-R8's early return-refund observation; E-T9 corrects the original `descendantsOf`
assumption; E-F10 closes the fulfillment negotiation-axis gap opened by E-F8).
...
- `read_all_orders` (full order history beyond 60 days) genuinely requires a **separate, manually
  ...
  **BLOCKED**, owner = Shopify review team, no ETA available. Recorded as open risk 1 below.
...
successful result on retry once the underlying resource state has moved — see the M-group findings
...
**F6 is confirmed as an even stronger match than the issue claimed, and the fulfillment negotiation
axis is now closed end to end.** Live introspection shows `FulfillmentOrderRequestStatus`
(`UNSUBMITTED, SUBMITTED, ACCEPTED, REJECTED, CANCELLATION_REQUESTED, CANCELLATION_ACCEPTED,
CANCELLATION_REJECTED, CLOSED`) and `FulfillmentOrderStatus` (`OPEN, IN_PROGRESS, CANCELLED,
...
grant, then re-verified working:

1. `write_merchant_managed_fulfillment_orders` — without it, essentially the entire write half of
   group F (`fulfillmentCreate`, `fulfillmentOrderMove`) was blocked with `"The api_client does not
...
| E-M10 | M5 | `products(query: "updated_at:>...", sortKey: UPDATED_AT)` confirmed working live, real `updatedAt` timestamps returned in the expected sort order | Live: 3 products returned with `updatedAt` in the expected range |
...
### S — Status, stock & commercial fields

| # | Story | Fact | Evidence |
|---|---|---|---|
| E-S3 | S6 | `ProductStatus` closed vocabulary: `ACTIVE, ARCHIVED, DRAFT, UNLISTED` — 4-value neutral publication status | Live introspection + live read on real product |
...
### X — Platform limits & multi-tenancy
...
**Deliberately skipped, not an oversight**: X6 (multi-tenant quota isolation) requires a SECOND
custom-distribution app, which can only be created through the Dev Dashboard UI — no
`appInstallations`/app-creation mutation is reachable from within an existing app's own credentials
(attempted: `appInstallations` query → `"access denied"`, requiring a broader scope this app was
never granted, by design — an app cannot enumerate its siblings). Structurally confirmed as correct
behaviour (an app should not be able to see other apps' installations) rather than a gap to chase
further.
...
# Token unchanged: shpua_REDACTED (still stable across re-auth).
...
1. **`read_all_orders` requires manual Shopify review** ("rolling basis", no stated SLA) via a
   ...
   request submitted, owner = Shopify, no ETA. Everything else in the story checklist can proceed
   without it since a fresh dev store has no orders older than 60 days anyway.
...
tally below — none of it is large enough to change the verdict, but AC2's "what the dev store cannot
