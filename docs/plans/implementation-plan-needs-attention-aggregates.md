...
1. **Extend `OrderRecordRepositoryPort` only** — deliberately matching the existing `countByHealth` precedent, not adding an `IOrderRecordService` pass-through
   ...
   - **Decision (resolved at the #1983 pre-implement gate, Finding 2)**: `IOrderRecordService` does **not** currently expose `countByHealth`, even though the repository port has it — the existing consumer, `apps/api/src/orders/http/orders.controller.ts`, injects `OrderRecordRepositoryPort` directly via its Symbol token for that exact read. `NeedsAttentionService` (Phase 4) follows the same precedent: it injects `OrderRecordRepositoryPort` directly, not `IOrderRecordService`. No `IOrderRecordService` change in this plan.
...
2. **Implement `NeedsAttentionService`**
   ...
   - **Action**: Resolve listing-capable connections once, pass to both `CoverageGapReadService` and `StockAtRiskReadService` to avoid resolving twice (per Phase 2 step 2 note). Inject `OrderRecordRepositoryPort` directly (via `ORDER_RECORD_REPOSITORY_TOKEN` from `@openlinker/core/orders`) for the failed-sync-value read — mirrors `OrdersController`'s existing pattern (Phase 3 decision). Call all three reads in parallel (`Promise.all`). Map to the response DTO.
   - **Verify early**: run `pnpm lint` right after adding the `OrderRecordRepositoryPort` import (before writing the rest of this file) to confirm `check-cross-context-imports` accepts an `apps/api` → `*RepositoryPort` import — `OrdersController` already does this today, so it should pass, but confirm before building on top of it (pre-implement gate Finding 3).
