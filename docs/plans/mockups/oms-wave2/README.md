...
So the `<script src="./support.js">` and `<script src="./ds-base.js">` tags at the top of each
template point at siblings that are **intentionally absent** — that is expected, not a broken export.
They are per-template runtime shims generated by the design app; committing them would still not make
the page render, because the bundle they bootstrap is the multi-MB one.
...
**Two scenario choices are deliberate, and are not the default state.** Read them as demonstrations
of a rule, not as what a fresh install looks like:

- **The returns row is shown in its ambiguous state.** `What happens to returned goods?` renders
  `OpenLinker can't tell` with the `Nothing is deciding` badge, and its why-line is A5-A's body copy
  rather than that row's default why-line (*"Nothing decides yet — you handle returns by hand."*).
  That is §3.3's badge-swap rule being exercised — the only way to show both the one red badge and
  the `Needs attention` row it implies, from a single fact. On a healthy install this row reads
  `Default` and `Needs attention` is empty, replaced by one line.
- **The attention card is amber while its table row is red.** One fact, two renderings (§3.3), but
  two severities: the table states what the decision currently *is*, the attention section states
  what the operator must *do*. §4.3 permits either `danger` or `warning` for an attention-worthy
  state, so this is a choice made inside the spec rather than a reading of it. Settle it before
  building — otherwise the four-value row-badge vocabulary (`Stopped` / `At risk` / `Blocked` /
  `Not matched`) loses its severity gradient.
