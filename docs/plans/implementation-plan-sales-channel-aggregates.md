...
**Explicit scope decision**: currency-mixing detection and gross/net tax-treatment normalization are **out of scope** for this plan. They are owned by #2049/ADR-040 (currency) and a separate tax-normalization effort respectively. `totalAmount` is summed as-is; see § 2 for the exact deferral statement this plan encodes in code.
