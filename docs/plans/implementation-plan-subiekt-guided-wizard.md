...
1. **Create the Zod schema + payload mapper**
   ...
   - **Action**: Export `SUBIEKT_ADAPTER_KEY = 'subiekt.invoicing.v1'`, `subiektSetupSchema` (fields: `name` required; `bridgeBaseUrl` **required** URL with `http`/`https` protocol — **not** an empty-string-optional union like Erli's `baseUrl`, since the bridge URL is mandatory; `timeoutMs` optional; `bridgeToken` optional), `SUBIEKT_SETUP_DEFAULT_VALUES`, `toCreateConnectionInput(values): CreateConnectionInput`. The mapper builds `config: { bridgeBaseUrl, ...(timeoutMs != null ? { timeoutMs } : {}) }`, sets `platformType: 'subiekt'`, `adapterKey: SUBIEKT_ADAPTER_KEY`, includes `credentials: { bridgeToken }` **only** when `bridgeToken` is non-empty, and omits `enabledCapabilities`.
   - **TR-IMPORTANT (numeric coercion)**: `timeoutMs` comes off an RHF text input as a **string**, but the BE DTO validates `@IsInt() @Min(1000) @Max(120000)`. Model it as a `z.union([z.literal(''), z.coerce.number().int().min(1000).max(120000)]).optional()` so blank → omitted and a typed value → **number**; the mapper must emit a `number` (never a string) into `config.timeoutMs`. (Alternatively, drop `timeoutMs` from the wizard entirely — it's editable later via #759's edit form — to keep the guided flow to name + bridge URL + optional token and remove this risk. Implementer's call; default to including it behind a clearly-labelled "Advanced" caption.)
   - **TR (bridge-URL rule)**: `http` is allowed (LAN bridge); the IMDS-safety guard stays **BE-authoritative** (a BE 400 surfaces in the create-error Alert). Do not duplicate the IMDS predicate as the FE source of truth.
   - **Acceptance**: `z.input`/`z.output` types exported; mapper unit-covered in the form test (asserting `timeoutMs` serializes as a number); no `any`.
   - **Dependencies**: none.
