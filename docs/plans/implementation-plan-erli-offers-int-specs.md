...
- `apps/api/test/integration/helpers/erli-test-offer-manager.helper.ts` — registers `erli.test.v1` adapterKey + a factory whose `createCapabilityAdapter(connection, …)` returns `new ErliOfferManagerAdapter(connection.id, 'erli.test.v1', fakeClient, { period: 2, unit: 'day' }, cachePort)` (the **real** adapter — constructor is `(connectionId, adapterKey, httpClient, defaultDispatchTime?, cache?)`; `identifierMapping` is **not** a ctor arg). Resolves `CACHE_PORT_TOKEN` from the harness for the 5th arg (`cache`); the 4th is a default dispatch time so creates don't fail closed. Returns a handle exposing the fake for scripting/asserting.
