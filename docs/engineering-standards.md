...
async findMany(filters, pagination)   // rows AND total - unchanged
...
- **`findMany` does NOT delegate to the two.** TypeORM's `getManyAndCount` calls `lazyCount` first, which infers the total with *no count query at all* when a page comes back short. Composing the combined read from the split one would add a statement on every small install.
...
**Exception — a vocabulary-only concern has no tokens file, until it needs one.** Rule 1 exists so a context's DI bindings are discoverable from one place. A concern that publishes only value types and pure guards — no service, no repository, no port, and therefore no NestJS module either — has no bindings to discover, so an empty `<ctx>.tokens.ts` would be ceremony and `export *` from it would widen the barrel with nothing.

`sales-documents` was this concern from #2100 (ADR-041 decision 1's "module now, context later" first slice) until #2170 gave its rule engine a real module + repositories; the exception ended there, exactly as designed — it now owns an ordinary `sales-documents.tokens.ts` with the usual sub-barrel `export *` line. That is the worked example, and the point of the paragraph: **the exemption expires the day the concern needs a binding.**

Two concerns hold it today — `fulfillment-authority` (#2304, [ADR-052](./architecture/adrs/052-independently-assignable-fulfillment-authorities.md) / [ADR-053](./architecture/adrs/053-fulfillment-authority-vocabulary-leaf.md)) and `order-lifecycle` (#2305, [ADR-059](./architecture/adrs/059-order-lifecycle-derived-phase.md)) — and ADR-053 is explicit that this is a **starting posture, not a permanent vow**: each grows an ordinary `<ctx>.tokens.ts` when its first binding arrives, exactly as `sales-documents` did. What must NOT lapse in the meantime is the separate, load-bearing property these three share — **zero sibling-core-context edges** (see `docs/architecture-overview.md § Cross-context dependencies in core`), which is what keeps a consumer's value-import CJS-cycle-safe and which `libs/core/src/__tests__/barrel-purity.spec.ts` enforces per leaf. Framework-freedom and sibling-edge-freedom are different properties; only the second is enforced, and #2170 is the proof they are separable.

**`fulfillment` (#2391) is a leaf that does NOT hold the exemption, and that is the rule applied rather than an exception to it.** The exemption's precondition is that a concern is *vocabulary-only* — "no service, no repository, no port, and therefore no NestJS module either" — and its expiry is "the day the concern needs a binding". For `fulfillment` that day is already named: #2392 lands `FULFILLMENT_WORK_REPOSITORY_TOKEN` with the `fulfillment_works` schema, and ADR-053 additionally places the A2/A3 authority-resolution services in this context. The two leaves above hold the exemption because on their ship day nobody could name their first binding; applying it here in order to delete it one PR later would be the ceremony, not the file. The file therefore ships with a header stating why it is empty, plus the ordinary sub-barrel `export *` line so the next slice cannot forget it — and an `export {};` so the module compiles, since a `.ts` file with no export statement is not a module and the star re-export would fail `TS2306`.
...
**The runtime constraint** (#591, #594): `libs/core/package.json` `exports` exposes only the top-level context barrels (`@openlinker/core/<ctx>`) plus the explicitly exported sub-barrels — `@openlinker/core/listings/services` (Nest wiring, #337/#359) and `@openlinker/core/<ctx>/orm-entities` (host-only ORM-entity access, #594). Deep paths like `@openlinker/core/<ctx>/domain/...`, `.../application/...`, `.../infrastructure/...` are **not exported** — they fail at Node runtime with `ERR_PACKAGE_PATH_NOT_EXPORTED`. ESLint guards this in `libs/integrations/**`, `libs/oms/**` (#2390) and `apps/{api,worker}/**`; the `orm-entities` sub-barrels carry an additional ban in `libs/integrations/**`, `libs/oms/**` and core port files so plugins never see TypeORM types.
...
**Sub-barrels** are the narrow exception to "top-level-barrel-only". They exist to expose a host-only seam that would otherwise pollute the contract surface plugins consume. Four families exist today:
...
- `@openlinker/core/<ctx>/types` — the **cycle-breaker** family: a context's dependency-free vocabulary, published WITHOUT its NestJS module, its services or its ORM entities. Today: `orders/types` (#2155), `invoicing/types` and `fiscalization/types` (#2515). It exists for a consumer that may not depend on the context at all - `sales-documents` is a zero-outbound-core-context-edge leaf, yet ADR-065 requires its per-order projection to NAME the existing invoice and fiscal status vocabularies rather than declare a third one. An `import type` from such a sub-barrel erases at build time, so it emits no `require()` and cannot close the CJS cycle `libs/core/src/__tests__/barrel-purity.spec.ts` guards; that spec lists the authorized specifiers, and `scripts/check-types-sub-barrels.mjs` bounds the other side, asserting a `<ctx>/types.ts` re-exports only from `./domain/types/**` so a repository port or ORM entity can never reach a consumer through it.
...
Add a new `<ctx>/types` sub-barrel only when a consumer genuinely cannot import the main barrel - a leaf context forbidden from depending on the source context, or a value-import cycle. Re-export from `./domain/types/**` and nothing else, and prefer type-only exports: a value export is what turns an erased edge back into a real one.
