...
- **Capability**: `InvoicingPort` (base — `issueInvoice` / `getInvoice` / `upsertCustomer` / `getSupportedDocumentTypes`) plus composable sub-capabilities in `domain/ports/capabilities/`, each with a co-located `is*` guard:
  ...
  - `CorrectionIssuer` (#1229) — issue a correction (`IssueCorrectionCommand`) of an already-issued document. `IssueCorrectionCommand.originalDocument` is an OPTIONAL, caller-assembled full reconstruction (buyer/currency/lines/clearance linkage) for adapters that cannot correct via a per-line delta and must resubmit a complete document (KSeF's FA(3) KOR, #1288) — the HTTP controller assembles it from the order snapshot (mirroring the keyless re-issue path) only when the resolved adapter needs it; adapters that only need deltas (Subiekt) ignore the field.
