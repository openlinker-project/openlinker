...
- **Not fiscalisation.** Registering a sale on a certified fiscal device or register is a different act with a different issuer, device dependency, legal basis and retry semantics; it is a separate capability, not an `InvoicingPort` document type - see § 16 below.
...
### 16. Fiscalisation (planned)

*See [ADR-042](./architecture/adrs/042-fiscalisation-capability.md) for the decision rationale.*
...
- **FiscalizationPort** (planned, #1908): hands a sale to a provider that performs or brokers its fiscal registration. Designed but not yet implemented - the contract, its sub-capabilities and their rationale are described once, in § Core Bounded Contexts, *16. Fiscalisation*.
