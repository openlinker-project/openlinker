...
### 16. Fiscalization (planned)

*See [ADR-042](./architecture/adrs/042-fiscalization-capability.md) for the decision rationale.*

- **Responsibility**: hand a sale to a provider that performs or brokers its fiscal registration, and surface the result on the order. No code exists yet — the capability is designed in ADR-042 and built in #1908. **OpenLinker never issues a fiscal receipt**: issuance is reserved to a registering device whose type carries a *potwierdzenie Prezesa GUM*, or to a software register meeting the same requirements, and OL stays on the feeding side of that line (#1906, art. 111 ust. 6a/6b applied *odpowiednio* by art. 111b ust. 2 of the PL VAT act). That states what the statute says and is **not legal advice** — no seller-facing compliance claim should rest on it without a professional opinion.
...
- **First adapter**: eparagony.pl (#1908, PL e-receipts) — a private e-receipt *distribution hub* sitting in front of a fiscal printer driven by the vendor's own software, not a fiscalizer itself. Status/link surfacing on the order is #1909 and the connection UI is #1911. Which document a given order gets - invoice or receipt - is the sibling routing decision ([ADR-041](./architecture/adrs/041-sales-document-routing-policy.md)), not this capability's.

### 17. Sales Documents

*See [ADR-041](./architecture/adrs/041-sales-document-routing-policy.md) for the decision rationale; decision 11's visibility contract is the shipped slice (#2100).*

- **Responsibility**: own the neutral vocabulary that answers *"why does this order have no fiscal document?"*, shared by the invoicing gate today and by ADR-041's routing decision once #1908 lands. It is above both document contexts by construction: a fiscal receipt is not an invoice, so neither `invoicing` nor the future `fiscalization` context can own the question.
- **Location**: `libs/core/src/sales-documents/` — types only. **No NestJS module, no service, no repository, no port**, and consequently no `sales-documents.tokens.ts` (the documented exception to `engineering-standards.md § Symbol DI Token Re-export Convention` rule 1).
- **Key types**: `SalesDocumentUnresolvedReason` (*routing could not decide*) and `SalesDocumentGateBlockReason` (*routing decided, or explicitly did not, and issuance is still not allowed*), kept as **two** unions because they answer different questions, with `'unresolved-routing'` as the one bridge value carrying the routing reason alongside it (ADR-041 §107). Plus `SalesDocumentBlock`, the `SalesDocumentBlockOutcome` the gate reports (`none` / `blocked` / `indeterminate`), `SalesDocumentAttentionReasonValues` (the aggregate-worthy subset), and the two `is*` coercion guards.
- **Zero outbound edges, enforced.** Both `invoicing` and `orders` value-import it — from a domain entity, a repository port, an application interface, an application service and an infrastructure repository — which is only safe because the concern imports nothing itself. `libs/core/src/__tests__/barrel-purity.spec.ts` fails the build on any `import … from` added under the directory.
- **Not yet declared here**: `SalesDocumentDecision` / `SalesDocumentKind` (the router's own types) are #1908-era work. See § Invoicing for how the shipped reasons reach `order_records` and the operator.
...
> The table below is a curated highlight. The **complete, code-synced inventory of all 36 sub-capabilities across every port** (with descriptions and guards) lives in [`docs/capabilities.md`](./capabilities.md).
...
- **FiscalizationPort** (planned, #1908): hands a sale to a provider that performs or brokers its fiscal registration. Designed but not yet implemented - the contract, its sub-capabilities and their rationale are described once, in § Core Bounded Contexts, *16. Fiscalization*.
...
`sales-documents` (#2100) is the opposite extreme and deliberately so: **a sink with zero outbound edges**. It publishes only value types and pure guards — no service, no repository, no port, and therefore no NestJS module — which is what lets both `invoicing` and `orders` value-import it from a domain entity, a repository port and an application service without any risk of a CJS module-load cycle. That property is pinned by `libs/core/src/__tests__/barrel-purity.spec.ts`, which fails on any `import … from` added under the directory, and it is also why the concern is the documented exception to the `<ctx>.tokens.ts` rule (`engineering-standards.md § Symbol DI Token Re-export Convention`).
...
- **`webhook_deliveries.status` is monotonic (#1916)**: the row is stamped by two deliberately decoupled writers - the ingress `WebhookService` (`received`, then `published` *after* the stream publish returns) and the `WebhookToJobHandler` that consumes that publish (`job_enqueued` / `deadlettered`, routinely within ~10 ms). Nothing orders the two, so `WebhookDeliveryRepository.upsert` resolves the `ON CONFLICT` assignment of `status` by an explicit precedence ladder (`WEBHOOK_DELIVERY_STATUS_RANK`: `received` < `published` < `job_enqueued` < the attention-worthy terminals `rejected`/`failed`/`deadlettered`) instead of by arrival order. Without it a fully-routed delivery reported as merely `published` while carrying a `downstreamJobId`, and the FE's `lastDeliveryResult` could not answer "did this event produce a job?". Every other overlay column stays last-write-wins (each has a single writer); the callers are untouched, since neither can reason about the other's timing.
