...
- **#2358 lands storage only.** Named siblings own the rest: #2359 the pure evaluator + the §5.4 legality matrix, #2360 trigger emission (and the firings writer), #2361 the six action executors, #2362 the at-most-one gate, #2363 the CRUD/evaluate/fired-log API, #2385 the `automation_runs` write path and the per-step outcome shape (the `sync_jobs` link for a step that dispatched a job rides inside that `steps` jsonb, not in a column), #2387 the AF-X attention state. The §5.6 90-day run-retention sweep is named in the spec and owned by no issue yet; `DemoAccountCleanupService` is the repo's one Postgres row-deletion retention precedent to mirror.
