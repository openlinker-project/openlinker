...
- **Which document a given order gets - routing policy (#2051, [ADR-039](./architecture/adrs/039-sales-document-routing-policy.md); Proposed, no code yet)**: `documentType` is command data the caller supplies (ADR-026 §Decision 3), so *selecting* it is a policy question that lives outside both document contexts - in a dedicated `sales-documents` core concern that reads the order and resolves **at most one** `(documentType, connectionId)` pair. Invoice **or** fiscal receipt, never both for the same order: that exclusivity is a contract-level invariant, and the unconditional `AutoIssueTriggerService` fan-out that violates it today is #2047. ADR-039 records the `orders <-> sales-documents` cycle-safety condition (types / Symbol tokens / service interfaces only), the `AutoIssueTriggerService` gating conditions (unresolved decision, missing required tax identifier, tax-rate conflict per #2009), periodic aggregation as a non-issuing outcome, and the self-routing-destination bypass. The rule engine and the localised legal matrix are deferred by the ADR itself; manual per-connection choice is the first slice.
