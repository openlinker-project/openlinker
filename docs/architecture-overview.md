...
- **Two pure resolvers, one live today.** `resolveSalesDocumentRouting` (#2155) is the `operator-configured` single-primary model `AutoIssueTriggerService` calls on every order transition — this is the only routing decision that reaches a real order today. `evaluateSalesDocumentRules` (#2170) is the country-agnostic 4-tier fallback evaluator (rule match → country default → `★` Rest of world → `unresolved`/`no-configuration-for-country`) backing the new Settings → Sales documents authoring surface (rules, per-country defaults, the Poland-only cited starter template, the no-`priority` conflict guard). **It is not yet wired into `AutoIssueTriggerService`** — an operator can author rules today, but they have no effect on real order routing until a follow-up connects the two resolvers (tracked as a gap, not a design decision; ADR-041 decision 5 itself notes the flagship Poland condition also needs `buyerTaxId` landing on the `Order` contract first).
