...
- **Capability**: `InvoicingPort` (base — `issueInvoice` / `getInvoice` / `upsertCustomer` / `getSupportedDocumentTypes`) plus composable sub-capabilities in `domain/ports/capabilities/`, each with a co-located `is*` guard:
  ...
  - `CorrectionIssuer` (#1229) — issue a correction (`IssueCorrectionCommand`) of an already-issued document. `IssueCorrectionCommand.originalDocument` is an OPTIONAL, caller-assembled reconstruction (buyer/currency/lines/clearance linkage) for adapters that cannot correct via a per-line delta and must resubmit a complete document (KSeF's FA(3) KOR, #1288). The HTTP controller builds it unconditionally (from the order's *current* state, mirroring the keyless re-issue path) whenever the backing order is still resolvable — it does not resolve the adapter first to check whether the snapshot is actually needed, since the extra order fetch is cheap relative to the correction call; adapters that only need deltas (Subiekt) simply ignore the field. Reconstructing lines from the order's current state (rather than a persisted issuance-time snapshot, which `InvoiceRecord` does not carry) is an accepted limitation — correcting an order whose items changed since the original issuance is unsafe until that snapshot exists.
...
- **Second provider**: **inFakt** (Polish accounting SaaS), `@openlinker/integrations-infakt`, adapterKey `infakt.accounting.v1`, registry capability `Invoicing`. Acts as a **KSeF intermediary** ([ADR-030](./architecture/adrs/030-infakt-ksef-indirection.md)): `issueInvoice` creates the document via `POST /invoices.json`, and inFakt auto-triggers KSeF submission on its own — OL never opens a KSeF session itself. `InfaktInvoicingAdapter` implements `InvoicingPort`, `RegulatoryStatusReader` (not `RegulatoryTransmitter` — there is no submit primitive for OL to call), and `CorrectionIssuer`. Clearance status is read via `GET /invoices/{uuid}.json` (`ksef_data.status`); inFakt's `send_to_ksef_success` / `send_to_ksef_error` webhooks (decoded via the ADR-021 inbound-webhook-decoder pattern) are a push-notification shortcut that triggers an immediate status re-read rather than being trusted as the system of record. See the [inFakt setup guide](./integrations/infakt/setup-guide.md) for connection setup, webhook configuration, and troubleshooting.
