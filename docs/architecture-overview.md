...
- **Deliberately NOT in this slice**: no return feed / `ReturnSourceReader` (#2329), no `return.decline` (#2333), no read API (#2334), no restock execution, and **no `shipments.direction`** — return labels are Wave 2, so ADR-060's note that `UQ_shipments_branch_one_per_order_conn` must gain `direction` in its predicate is carried forward rather than actioned.
