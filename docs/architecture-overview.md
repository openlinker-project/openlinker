...
The two new leaves appear in the map above as **bare nodes with no edges in either direction**. That is accurate rather than an omission: they are sinks by construction, and nothing consumes them yet — their first inbound edges arrive with the waves that adopt the vocabulary. A leaf gaining inbound edges is the expected evolution; a leaf gaining an **outbound** one is the failure the spec exists to catch.
