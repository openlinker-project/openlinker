...
- **Bounds are enforced server-side, in one table.** `OPERATIONAL_SETTING_BOUNDS` is read by the API validator, by the response DTO's `bounds` block (so a form derives its `min`/`max` rather than restating them) and by the worker's own clamp - reported === enforced structurally, the #2229 rule applied to a settings surface. A DTO decorator is a convenience for the OpenAPI document and a fast rejection, never the gate: the browser is not a trust boundary and the raw JSON path bypasses any form, mirroring `ConnectionService.validateRateLimitConfig`. Two ceilings deviate from what #2651 proposed, deliberately: the **sweep page size caps at 100, not 500**, because that is the existing `SWEEP_BATCH_SIZE_MAX` (PrestaShop's own collection page, WooCommerce's hard `per_page` cap) and accepting 500 while the handler clamps to 100 would be exactly the reported-versus-enforced gap the table exists to close; and the **inventory budget's ceiling of 2000 is passed into `resolveSweepBudget` at the call site**, replacing the legacy `SWEEP_BUDGET_MAX` of 500 on that path, so a number an operator is told is accepted is the number that runs.
