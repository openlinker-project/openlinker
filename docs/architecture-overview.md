...
- **The catalogue sweep fans out one child per PAGE, not per product (#2593, [ADR-048](./architecture/adrs/048-incremental-catalog-replication.md) decision 1)**: `PrestashopProductMasterAdapter.getProducts` had always done `display=full` plus batched identifier mapping - 100 fully-hydrated products in 3 requests, 0.38 s, 1.33 MB, measured live - and had no caller anywhere in the repo. It could not have one: the sweep fanned out one `master.product.syncByExternalId` per product, and `getCapabilityAdapter` constructs a fresh adapter per call, so a per-product child could never share a bulk read no matter how cheap the read was. The sweep now enqueues `master.product.syncBatch`, one child per page of products, and that child runs the per-product loop through ONE adapter instance. Five properties are load-bearing. (1) **It is a PREFETCH, not a second sync path.** The new guard-only sub-capability `BulkProductReader.prefetchProducts` warms whatever the adapter caches and returns nothing; `IMasterProductSyncService.syncFromMasterByExternalIds` then runs the unchanged per-product body per id, so identifier mapping, variant resolution, the tax-rate journal, the #1904 rival-claimant prune guard and the #1599 deletion signal all still happen exactly where they did. A path returning hydrated products would have had to re-implement every one of them, and a guard re-implemented is a guard that drifts. The consequence is that the prefetch must be **invisible except in request count**: a master declaring nothing, or one whose prefetch throws, degrades to the reads it always issued. (2) **A failed product does not fail the page.** The service reports per-id failures instead of throwing and the handler re-enqueues each as an ordinary per-product job, keeping the retry ladder and the dead row the fan-out always had - otherwise one poisonous product would take its ninety-nine page-mates down with it, ten attempts deep. (3) **A deletion is logged rather than reported as the job's outcome**, because a mixed page cannot carry the per-product `business_failure` / `master_deleted` label. That costs nothing real: this path is fed by an enumeration read FROM the master, which is structurally blind to a deletion (decision 2), and `master.product.reconcile` - still a per-product fan-out - remains the deletion authority and keeps the label. (4) **Two adapter-side fixes were prerequisites, both silent before.** `filter[id]` joined ids with a comma, which PrestaShop reads as the RANGE between the lowest and highest requested id rather than an OR list, so a page of sampled ids returned most of the catalogue; it is pipe-joined now. And the builder emitted no `sort`, so offset paging trusted whatever order MySQL returned. (5) **The budget was re-derived, and deliberately not raised further.** `BATCHED_SWEEP_BUDGET_DEFAULT = 500` items per run at 100 per batch, against the same drain-rate rule the per-product 100 came from - the unit is now a page hydrated in three requests rather than a child job spending four. The **cadence is unchanged at 20 minutes**, which is the honest half: past this budget the ceiling is the lane's per-scope cap, not the shop, so a shorter cron would queue work the runner cannot drain any faster. The batch child runs in the **`bulk`** lane, not `realtime`: it is a catalogue-sweep child, and ADR-050 picks a lane by cost of starvation, which is exactly why #2594 moved the per-product sweep children (`master.product.syncFromSweep` / `master.inventory.syncFromSweep`) out of `realtime` in the first place. So `OL_LANE_BULK_SCOPE_CAP` governs it, and a catalogue cycle can never fill the realtime lane's slots ahead of a buyer's order.
- **Deletion reconciliation - `master.product.reconcile` (#2222, [ADR-048](./architecture/adrs/048-incremental-catalog-replication.md) decision 2)**: the deletion authority for the catalog, and the counterpart to the sweeps above. A catalog enumeration **cannot** reveal a deletion - `master.product.syncAll` reads ids FROM the master, so a deleted product simply stops appearing and no child ever runs for it. This pass inverts the direction: it enumerates **OL's own `Product` identifier mappings** (paged via `listExternalIdsByConnection`, #2219), filters synthetic variant ids, and enqueues `master.product.syncFromSweep` (the existing per-product sync body; #2594 split the type so the child lands in the `bulk` lane rather than `realtime`) - so a live product re-syncs idempotently and a deleted one raises `MasterProductNotFoundError` into the deletion path. That is precisely the shape `master.inventory.syncAll` has always had, which is why an `InventoryMaster` connection already had deletion coverage and a `ProductMaster`-only one had none. **Absence is never the signal, and must not become one**: stable pagination is not guaranteed on every adapter (`PrestashopProductMasterAdapter.listExternalIds` sorts `id_ASC` since #2627; `WooCommerceProductMasterAdapter.listExternalIds` still sends no `orderby`), so a cycle spanning many ticks can miss a LIVE row after a mid-cycle delete shifts the offsets - and staling on that inference would zero that product's offers on every marketplace via #1689. Because the child is the authority the pass carries **no guards of its own** (an empty enumeration enqueues nothing; a missed mapping is re-checked next cycle; the #1904 guard lives where the write is), takes its own lock and cursor namespace (`product-reconcile`, a fourth `SweepKind`) so the full sweep cannot starve it, and uses its own child idempotency namespace so a re-check never dedups against a sweep or delta child. **Default ON**, unlike the opt-in delta pass (#2220) - it closes a live defect where a deleted product's offers kept selling, so every existing `ProductMaster` install picks it up on deploy as new recurring platform load. Hourly, and **not** capability-gated to connections lacking `InventoryMaster` - a capability-shaped exclusion rots and would skip the #1904 case - at a steady-state cost of ~14% more per-product children on a connection carrying both. Note the **cron is the tick, not the cycle**: a cycle spans `ceil(N / budget)` ticks, so detection latency is ~`ceil(N/100)` hours at the defaults - about 41 days on a 100 000-product catalogue, which is what #2647 addresses next.
- **A master may also TELL us (#2647, [ADR-048](./architecture/adrs/048-incremental-catalog-replication.md) § Amendment (#2647))**: PrestaShop module 1.9.0 registers `actionProductDelete` and writes a `product.deleted` row through the existing outbox, so a deletion reaches OL through the ordinary webhook path instead of waiting for the audit. **The event is a trigger, never an assertion** - the translator decodes it into the `product` domain like any other product event, it routes to the same `master.product.syncByExternalId` job, and **the master answering not-found is still the authority** that stales the variants, emits `master.product.stale` and pauses the offers; a rolled-back delete therefore costs one redundant re-sync and nothing else. Verified end to end on a 100 000-product stand (`perf/prestashop-baseline/results-E-2026-08-28.md`): outbox row written synchronously with the delete, job terminal as `master_deleted`, 3 of 3 variants `isStale`. **Detection latency becomes the shop's cron cadence** - roughly a minute and a half on a one-minute cron, against the audit's derived ~41.7 days. What this changes is that `master.product.reconcile` is now the **backstop** rather than the only route; it is NOT weakened and keeps its cadence, because a webhook is at-most-once at the shop's discretion and the audit is what survives a lost delivery, a disabled hook, a shop that never upgraded the module, and every master with no delete hook at all. A deletion is also verified not to be swallowed by a page of 100 on the batched sweeps: the prefetch seeds an empty bucket for every id it asked about, so a deleted product is a cache *hit* with zero rows rather than a miss and the per-item deletion path runs unchanged.
...
- **Operator controls, a zero threshold, and the cancellation hole (#2610)**: both `stockSafetyBuffer` (#1844) and `pricingRule` (#1843) were implemented, tested and unreachable - `apps/web` carried no field for either, so they protected and priced nothing. The connection Edit page now carries a platform-neutral `StockAndPricingSection` beside `RateLimitSection` (#1810), each group behind its own toggle and each stating its effect on a worked example rather than only taking a number, because a mis-set margin or reserve silently changes what buyers pay or how much you appear to have. Three properties are load-bearing. (1) **`0` and unset are different persisted states and must round-trip apart** - a typed `0` is the operator saying "no reserve", writes a real `0`, and reads back as `0` - and because the form recommends exactly that, `0` is **not** treated as a probable typo by `isPresentButInvalidStockSafetyBuffer`, which would have had the product recommending a value and the log calling that same value suspect on every write-back; a cleared knob writes an EXPLICIT `null` rather than deleting the key, for the same shallow-spread reason as `rateLimit` (#2016), and every reader already treats `null` exactly like absent. (2) **A margin of 100% or more is refused, not accepted and silently degraded**: `applyPricingRule` falls back to the base price there, so an operator who typed 120 would save happily and publish an unchanged price. The refusal is **server-side as well as in the form**, alongside `validateRateLimitConfig`'s neutral bounds check in `ConnectionService.create`/`update` - the form is not the only way in, since the raw JSON editor sits on the same page and bypasses the form's own `superRefine`, and curl and MCP bypass the browser entirely. The same neutral check bounds `stockSafetyBuffer` and `stockZeroThreshold`. The browser cannot import `@openlinker/core` (#591), so the worked examples run a deliberate mirror (`features/connections/lib/stock-and-pricing-preview.ts`) kept byte-identical by `scripts/check-stock-and-pricing-preview-mirror.mjs` under `pnpm check:invariants` - a preview computed by different arithmetic than the publish path is worse than none. (3) **`stockZeroThreshold` is the second remedy the #1844 design named**, for slow-moving stock: a quantity strictly below the threshold publishes as `0`. It is applied AFTER the reserve, to the quantity the destination actually sees, because the threshold is a statement about that number and not about the master's own count; `0` disables it, so an existing connection is unchanged. It carries its own present-but-invalid guard (`isPresentButInvalidStockZeroThreshold`), because a mistyped threshold fails in the worse direction than a mistyped reserve - the destination sells the low stock the operator asked it to hide - and the `listings` stock-at-risk aggregate (#1983) resolves its "publishes nothing" predicate through `applyStockSafetyBuffer` rather than the reserve alone, or it would under-report precisely the lines the threshold silenced. The same change closes a live defect on the cancellation path: `OfferStockRestoreService` wrote the raw master count as an ABSOLUTE quantity and so **wiped the reserve on the first cancellation** until the next full sync. It now reproduces the same publish policy the ordinary write-back applies, which is also what keeps a threshold `0` a deliberate operator choice rather than a post-sale hold.
...
- **Batched stock read (#2648, [ADR-048](./architecture/adrs/048-incremental-catalog-replication.md) decision 1)**: the inventory sweep now enqueues one `master.inventory.syncBatch` child per PAGE instead of one `master.inventory.syncFromSweep` per product, and `InventoryMasterPort` gains the optional `BulkInventoryReader` prefetch rung - the same shape #2593 gave the catalogue sweep, deliberately copied rather than reinvented. It was the last per-product fan-out on a master path, measured at **100 requests per 100 stock positions** against the catalogue sweep's 5.8, i.e. ~10.4 days for one cycle of a 100 000-product catalogue. Four properties carry over unchanged and are load-bearing for the same reasons stated under *Products*. **The rung is a PREFETCH, not a second sync path** - void-returning, best-effort, and invisible except in request count, so the variant-keyed write (#822/#823), the #1904 rival-claimant guard and the #1688 deletion signal all keep running exactly where they do, unduplicated; a master declaring nothing, or one whose prefetch throws, gets the per-product reads it always had. **The adapter is resolved ONCE for the page**, which IS the mechanism: `getCapabilityAdapter` builds a fresh adapter per call, so a per-product child could never benefit from a bulk read however cheap. **A failed product does not fail the page** - the service reports failures and the handler re-enqueues each one as a per-product `master.inventory.syncFromSweep` job under a `batchRetry` key, so that job type stays registered and keeps its own retry ladder and dead row. And the child runs in **`bulk`** (#2594/ADR-050), never `realtime`. The PrestaShop implementation reads `stock_availables` filtered by a **pipe-joined** `id_product` OR list with an explicit `sort` - a comma is a RANGE, which answers with a near-whole-catalogue page and nothing for the ids in the middle, and an unsorted offset read has no tiling guarantee at all; both are asserted against the string the real query builder emits rather than against a mock's arguments. Two things this deliberately does NOT do: it does not raise the sweep's budget or touch its cron (making the child cheap is the prerequisite, choosing what the run may then afford is a separate decision, taken with the same question for the deletion audit), and WooCommerce is not required to declare the rung.
- **Ordered quantity writes per offer (#2617, [ADR-067](./architecture/adrs/067-freshness-token-write-ordering.md))**: the `realtime` lane runs jobs concurrently and promises nothing about the order two `marketplace.offerQuantity.update` jobs for the SAME offer finish in, so an older quantity could land last and leave the listing advertising stale stock until the next change at the master - overselling when the older number is the higher one. Every propagation now carries the inventory row's own write stamp (`inventoryUpdatedAt`, already minted by `InventoryService.setInventory` for the dedupe key) end to end as `UpdateOfferQuantityCommand.observedAt`, and `InventorySyncService` refuses a write whose observation is strictly older than the newest one already written for that offer. Four properties are load-bearing. **The mark advances only AFTER a successful marketplace write**, so a refusal always means a newer quantity is live on the channel - a newer write that failed cannot lock an older one out, which is what keeps the guard from being a way to leave a listing permanently stale. **Only strictly older is refused**: an equal observation is a job retry and is idempotent, and an unparseable value on either side passes, so a guard that cannot compare degrades to the pre-#2617 last-write-wins behaviour rather than dropping stock updates. **A short per-(connection, offer) `SyncLockPort` lock wraps read-compare-write**, because the comparison alone leaves two windows open - a non-atomic read-then-write of the mark, and two marketplace calls in flight whose arrival order at the platform OL does not control. Contention is reported as a `write_contended` failure and the handler turns an all-contended result into a neutral `ContendedWriteError`, which the runner **defers penalty-free** under the bounded deferral budget below - contention is the guard working, not the job failing, so it must not spend a retry attempt and eventually dead-letter the very stock write the guard exists to protect. `advanceCursor` is a plain set rather than a compare-and-set, so the lock is load-bearing for the MARK's monotonicity too, not only for ordering the marketplace calls. **`observedAt` is optional**, so the stale-offer pause (#1689), which zeroes a listing on its own authority with no observation to quote, keeps writing unguarded, and a legacy job queued across the deploy behaves exactly as before. The mark lives in the ordinary `connection_cursors` store keyed `inventory.offerQuantity.observedAt:offer:{offerId}` - durable across restarts and shared by every replica with no new table, at the cost of one cursor row per mapped target per connection - never deleted, so a removed mapping leaves its row behind (an accepted leak, bounded by target count). The key says `offer` but also holds `ShopProduct` external ids on the shop write-back branch, which is safe only because the namespace is per connection and a connection carries one mapping kind. **The token must come from ONE clock, which is a precondition rather than a hope**: `observedAt` is `inventory_items.updatedAt`, which #2071 made database-stamped and read back via `RETURNING`, so cross-process skew does not exist and the only ambiguity is two writes inside the same millisecond, which the rule lets through. Replacing that stamp with a worker-side `new Date()` would let a process running ahead refuse a genuinely later write and leave the channel stale - worse than the last-write-wins this replaced. The guard costs no extra marketplace call - an observed batch is guarded PER ITEM and the survivors still go out in one `updateOfferQuantitiesBatch` call, with only the ids the adapter reported as succeeded advancing their mark - and it does not depend on lane serialisation - which matters because #2609 removes the accidental single-slot ordering the `fan-out` lane provided. **Narrowed by #2621, [ADR-067 § Amendment (#2621)](./architecture/adrs/067-freshness-token-write-ordering.md#amendment-2621-successful-write-narrowed-to-successful-submission)**: once `AllegroOfferManagerAdapter.updateOfferQuantity` returns on Allegro's async ACCEPT rather than a terminal status, "a successful marketplace write" for Allegro means *submitted*, not *applied* - a submission that later fails asynchronously has already advanced the mark, with no automatic re-drive (the reconcile pass's `'failed'` outcome only records the status; the inventory sweep is the only backstop), and the lock only orders *submission*, never Allegro's own application order for two already-accepted commands.
...
- **Net Sales, VAT-exclusive (`revenue.net*` fields, net-sales tax-rate epic, [ADR-063](./architecture/adrs/063-per-line-tax-rate-resolution-and-provenance.md) § Consequences)**: alongside gross `revenue`, `getSalesAndChannelAnalytics`/`getDailyOrderAggregates` compute a VAT-exclusive `netRevenue` per bucket, plus `netExcludedCount`/`netExcludedValue` for what a stored-rate requirement leaves out. An order is net-eligible only when it is not `taxRateEra = 'pre-rollout'` history, carries at least one line, and — for gross-priced (`taxTreatment` absent/`inclusive`) lines — every line's `order_line_items.taxRate` resolves to a known fraction (`resolveNetSalesTaxRate` / its SQL twin `netSalesRateFractionSql`, `libs/core/src/orders/domain/types/net-sales-tax-rate.types.ts`); net-priced (`exclusive`) lines need no rate, since the stored `unitPrice` is already VAT-exclusive. **Net is `gross / (1 + rate)`, never `gross × (1 - rate)`** — the latter understates net by `gross × rate²/(1+rate)` and was a shipped bug caught only by an int-spec computed against a real fixture, not by the unit specs that had been (wrongly) asserting the buggy figure.
- **Operator opt-in for backfilled pre-rollout orders (#2469, epic #2452 Phase 5, amending [ADR-063](./architecture/adrs/063-per-line-tax-rate-resolution-and-provenance.md) § Consequences — "no such figure reads the marker yet" is superseded by this)**: ADR-063's blanket pre-rollout exclusion is deliberately conservative — a `taxRateEra = 'pre-rollout'` order never enters Net Sales even when `TaxRateBackfillService` has since written it a real rate. `analytics_display_settings.include_backfilled_tax_rates_in_net_sales` (read by `apps/api`, threaded in as a plain `includeBackfilledPreRollout` boolean — see the `analytics --> orders` cross-context-edge note above for why it is a parameter rather than an `orders --> analytics` import) makes the **era clause alone** vacuous (`netSalesEraEligibleSql`); it does **not** relax the rate-resolution requirement, so a pre-rollout order is admitted only when a real rate has actually been written to its lines. **Turning the setting ON does not retroactively pull in an order whose rate is merely resolvable from the live catalogue but never written** — net-sales arithmetic needs a stored rate to multiply by, and there is no SQL equivalent of a live catalogue read; that is what `POST /analytics/coverage/tax/rerun-backfill` (coverage category C) exists to close, by actually running the backfill. **No `order_records` row is mutated by flipping the setting** — it is a pure query-time gate, instantly reversible in both directions, which is why the flag is a required parameter (never a repository-held default) on both `netSalesOrderNetEligibleSql` and its per-line sibling `netSalesLineNetEligibleConditionSql` — a caller that forgot to thread it would silently keep reporting the pre-#2469 figure while the UI claimed the setting was on. The same flag is threaded into `TaxCoverageDetectionService`'s category counts, or an order the setting just admitted into Net Sales would keep being reported as outstanding category-A work.
...
- **Non-blocking quantity acknowledgement (#2621)**: `AllegroOfferManagerAdapter.updateOfferQuantity` / `updateOfferQuantitiesBatch` no longer block on Allegro's terminal quantity-change status — they return as soon as Allegro *accepts* the write (`QUEUED`/`ACCEPTED`), and only a synchronous `REJECTED` (a definitive answer known at submit time) still throws. The "confirm later" half is the optional `PendingQuantityAckReconciler` sub-capability (advertised-without-dispatch, mirroring `OfferCreator`/`CategoryBrowser`): `reconcilePendingQuantityAcks(limit)` reads the connection's outstanding `allegro_quantity_commands` rows and re-checks each *distinct* Allegro `commandId` once — a batched write (#2622) persists several rows under one shared commandId, and Allegro's status read answers for the whole command, so resolving every row from that one response (rather than one GET per row) avoids re-fetching the identical response once per offer. Remaining fan-out across distinct commandIds is bounded by the same operator-configurable `resolveBatchConcurrency` ceiling the EAN-resolve batch path uses (ADR-047/#2229), never an unbounded `Promise.all`. Reconciliation never throws for an individual command's outcome — a genuinely failed command is persisted `'failed'` on `allegro_quantity_commands` and warn-logged, since there is no caller left waiting to catch it; only a whole-pass infrastructure error propagates. `IOfferQuantityAckReconcileService` (core) + the `marketplace.offerQuantity.reconcile` worker job + scheduler task (`allegro-quantity-ack-reconcile`, default ON, `*/2 * * * *`, page limit env `OL_ALLEGRO_QUANTITY_ACK_RECONCILE_PAGE_LIMIT`) drive the reconcile pass per connection; the job reports `outcome: 'ok'` regardless of individual command outcomes, matching every other sweep-shaped job in the repo.
...
- **Concurrency lanes (#2278, [ADR-050](./architecture/adrs/050-workload-isolation-concurrency-lanes.md))**: the worker runner schedules jobs through four **lanes** — workload profiles chosen by cost-of-starvation (`realtime | bulk | fiscal | fan-out`), never by I/O shape or bounded context. A job's lane is declared at handler registration (`SyncJobHandlerRegistry.register(jobType, handler, lane)` — a required parameter), and registration ends with a boot assertion that the partition covers **every** `JobTypeValues` member: lane-aware claiming selects rows by `"jobType" = ANY(<lane membership>)`, so an uncovered type would otherwise be silently stranded in `queued` (worse than the loud `markDead` an unhandled type used to get). Each tick every lane with free slots claims independently via `findAndLockDueJobsForLane` (same `FOR UPDATE SKIP LOCKED` shape, additive `jobType`/scope arms — no schema change) and jobs run **concurrently** under per-(lane, scope) slot accounting, `scope = connectionId` today via the pure `resolveJobScope` seam (ADR-050 decision 3). Never strict priority: a saturated `bulk` lane (an operator's 1000-child publish wave) cannot delay a queued `realtime` or `fiscal` job beyond its own lane's availability. Scopes at their per-scope cap are excluded **in the claim** (their rows stay `queued` — no lock/release churn), and the one case the exclusion cannot cover — a single claim returning several same-scope jobs — is trimmed post-claim, the surplus released back without an attempt penalty. Cap values (`OL_LANE_{REALTIME,BULK,FISCAL,FANOUT}_CAP` + `_SCOPE_CAP`, documented in `apps/worker/.env.example`) are env-overridable **illustrative defaults** until #1134 supplies per-lane measurements (ADR-050 decision 6) - with one exception, `bulk`, described next. **A job's lane depends on its TRIGGER, not only on its type (#2594, ADR-050 § Amendment).** `master.product.syncByExternalId` is a webhook-driven single-product sync someone waits on, and was also the child the catalogue sweeps enqueued a budget (100) wide per tick - so one type carried two costs of starvation, and the mapping was wrong in both directions at once: a catalogue cycle filled the `realtime` lane's per-scope slots ahead of a buyer's order, *and* the catalogue was throttled to `OL_LANE_REALTIME_SCOPE_CAP` (default 2), a cap sized for waited-on work. Measured on a live PrestaShop shop, raising the connection's own rate limit from 60 to 300/min moved traffic from 50 to only 63 req/min - the limiter was never the ceiling. The fix keeps the lane declared per job type and **splits the type**: `master.product.syncFromSweep` / `master.inventory.syncFromSweep` are the sweep-triggered children, registered in `bulk` against the SAME handler instances as their `…syncByExternalId` twins. A fifth lane was rejected because ADR-050 decision 1 makes that a reversal gate, and both types stay registered so a child queued under the old type across the deploy still runs and the boot coverage assertion still holds; an operator can now also tell catalogue work from webhook work on the Jobs surface. Consequently **`bulk` is the first lane whose caps are measured rather than illustrative**: `total: 12, perScope: 8` (from `2 / 1`), from an interleaved A/B run that sustained ~277 req/min while ~12 children ran concurrently on one connection, taking a 39 700-request catalogue from ~26.5 h to ~2.4 h. **The p95 response ratios that run reported are withdrawn and must not be quoted** - ADR-066 correction 1 records that the probe never checked HTTP status, so a fast error under load counted as a fast sample, and correction 2 records that store impact varies with cursor DEPTH rather than with catalogue size. The throughput figure stands; the store-impact figure does not, so `OL_LANE_BULK_SCOPE_CAP` should be lowered for a shop whose own latency moves. Three limits on that number are load-bearing: it covers the **PrestaShop catalogue read path only** (lower `OL_LANE_BULK_SCOPE_CAP` for a slower shop); it **speeds up every other `bulk` job type**, the offer-create and shop-publish waves and the status-sync/backfill sweeps included, which is intended but is a global change; and `perScope` sits deliberately below the measured ceiling because decision 4 ships no round-robin fairness, so at `perScope === total` one connection's cycle could hold the whole lane. `realtime` and `fiscal` keep their untuned defaults - the point of moving the sweep child out is that the buyer-facing lane did not need raising. **`fan-out` did have to be raised, but for the opposite reason: not a wrong lane, a wrong SCOPE (#2609, ADR-050 § Amendment (#2609)).** `inventory.propagateToMarketplaces` was enqueued with a synthetic `00000000-0000-0000-0000-000000000000` connection id, so every stock write in the installation shared one scope, and a `fan-out` per-scope cap of 1 then serialised all of them - measured on the demo stack as a queue growing ~145 jobs/h faster than it drained, with 15 066 rows backlogged from ordinary operation. The job now carries **the master connection the stock was read from** (`IInventoryService.setInventory(item, sourceConnectionId)`), which is what makes per-scope accounting mean anything, and `fan-out` defaults to `total: 8, perScope: 4`. The lane itself is **confirmed, not changed**: propagation makes no marketplace call of its own, and #2594's split-by-trigger has nothing to separate here because a webhook and the inventory sweep both discover real stock drift - on a master with no stock webhook the sweep is the only thing that discovers it at all - so the cost of starvation is one cost, not two, and the job type stays single. The cap of 1 was sized for the lane's other, **cron-paced** members; propagation is **event-paced**, one job per changed stock row. Raising it also lets two connections poll or enumerate concurrently, which the old cap prevented across the whole install, and it cannot multiply catalogue fan-out because each sweep additionally holds its own per-(kind, connection) `SyncLockPort` lock. The raise is a cap on database reads and child enqueues rather than on outbound requests, which is why it needed no per-destination measurement the way `bulk` did. **The all-zero placeholder id survives as a dormant fallback, and is now readable as one (#2745)**: `InventoryService` still stamps `00000000-0000-0000-0000-000000000000` when no source connection is supplied, and `MasterDeletionToJobHandler` stamps it on every `marketplace.offer.pauseStale` job it enqueues - a job that genuinely has no owning connection. The frontend used to fetch `GET /connections/:id` for it, take the 404, and render `Unknown`, which is exactly what a deleted or broken connection renders as; it now renders `System` with no id and no link. The constant is hardcoded independently in all three places (the browser bundle cannot import `@openlinker/core`, #591), so `scripts/check-system-connection-id-mirror.mjs` under `pnpm check:invariants` fails the build if any copy drifts - otherwise the frontend special-case would silently stop firing and the row would quietly revert to `Unknown`. **The caps bound one worker PROCESS, not the deployment**: slot accounting is in-process, so N replicas multiply every effective cap by N — deliberately unlike the globally-enforced pg-boss `groupConcurrency` precedent ADR-050 decision 3 cites, since cross-replica enforcement is out of scope for Wave 3 (multi-replica *correctness* is still the existing `FOR UPDATE SKIP LOCKED` claim's job — only the pacing is per-process). Per-job machinery (retry ladder, heartbeat, rate-limit penalty-free requeue) is unchanged — stuck-job recovery moved out of the runner entirely in #2279, to the `maintenance` role; the legacy un-laned `findAndLockDueJobs` is retained for its non-runner consumers.
...
- **Per-connection sync-queue status (#2615)**: a read-only view answering "is this connection's work getting done", rendered on the connection detail page's health tab via `GET /connections/:connectionId/sync-status`. It lives **in `sync` rather than in a third trust context** (`analytics-trust` / `catalog-trust`): those two exist because they compose FOREIGN contexts that no single owner can answer for, whereas every input here - `sync_jobs`, `connection_cursors`, the backlog rule - already belongs to `sync`, so a wrapper context would add a hop and own nothing. It also resolves **no adapter and makes no outbound call**, which is a requirement rather than an optimisation: the diagnostics must answer precisely when the shop does not. Four properties are load-bearing. (1) **The alert threshold is derived from the connection's own drain rate, never a fixed number** - `alertThresholdJobs = drainRatePerHour * 24 h`, measured over a one-hour observation window, because 500 queued jobs is a crisis on a shop that clears 20 an hour and twenty minutes of work on one that clears 1500. The ~145 jobs/h net growth the #2590 campaign measured is one seeded catalogue under a load harness on one machine, not a typical install, and is deliberately not encoded anywhere. (2) **Alerting requires four conditions together**: the queue is not converging (arrival > drain), the DUE queue holds more than the derived threshold AND more than an absolute floor (`BACKLOG_MIN_ALERT_JOBS`), its oldest due job has already waited longer than the alert horizon, and something actually succeeded inside the window. The wait condition keeps the alert honest about sweeps - an operator triggering a full sweep enqueues thousands of jobs in one minute, satisfying the first two while nothing is wrong, so a fresh deep queue reads `growing` and only becomes `backlogged` once the wait is genuinely a day old. The floor and the measured-drain condition keep it honest about **retries**: a failing job is requeued to `queued` with a future `nextRunAt` and its original `createdAt`, so with a measured drain rate of 0 the derived threshold is 0 and "more than the threshold" degenerated into "the queue is not empty" - one retrying job on a quiet connection raised the red banner while the worker was healthy. Queue depth therefore counts **only DUE jobs** (`nextRunAt <= now`), with backing-off jobs reported separately as `deferredCount`, because a job waiting on its own schedule is not queue depth. An alert that fires on a healthy install is worse than no alert. (3) **Vocabulary follows the `catalog-trust` standard**: `idle | draining | growing | failing | backlogged | unknown`, where `unknown` means the counts could not be read (never a healthy-looking zero), and a clearance estimate is `null` when the queue is not converging rather than a very large number dressed up as an estimate. `failing` exists because **only success is drain**: counting `dead` as drain gave a connection whose every job fails fast a high drain rate, a high threshold and an empty queue, so it read green while nothing worked at all. Nothing succeeded in the window plus at least one death now reads `failing` whatever the queue depth is. A connection holding no cursor reports `null` and is described as normal for a webhook-fed connection, not flagged. (4) **The attempt-duration mean excludes NULL** `lastAttemptDurationMs` (#2611) and reports its own non-null sample size beside it - the column is null on every row predating its migration, so counting those as zero would understate every real duration - and the read offers no SUM at all, because the column describes ONE attempt and a total time spent syncing cannot be built from it. Read cost is **two bounded aggregates** plus one single-row cursor read. The split is what bounds it: `sync_jobs` has no retention anywhere in the tree, so a single unbounded aggregate touched every row the connection ever had - millions inside a year on a connection taking sweep children every twenty minutes. The live-queue aggregate is restricted to `status IN ('queued','running')`, so it scales with the live queue rather than with history and is served by the `(status, nextRunAt)` index; the historical aggregate is bounded by `createdAt >= now - BACKLOG_HISTORY_WINDOW_MS` (7 days, far longer than the retry ladder's ~2-day maximum, so no job can complete outside it and be missed as drain) on the `(connectionId, createdAt)` index. `deadCount` is windowed the same way, so an old death clears with age instead of keeping a note on the panel forever. Response size is independent of job count either way.
- **Penalty-free deferral, and the bound that keeps it finite (#2613 / #2617)**: some failures are not the job's own. A destination throttling us (429), a destination under maintenance (503), a queued rate-limit slot that timed out, or a write refused because a peer held the serialisation lock - none of those is evidence the job is broken, so each requeues **without consuming a retry attempt** (`requeueWithoutPenalty`), through one path in the runner. A platform adapter reports the first two through the optional `RetryClassifierPort.getRetryDeferral`, probed rather than trusted (the ADR-046 shape) and coerced at the registry, which ignores a non-positive or non-finite delay and clamps anything above one hour so a buggy plugin cannot park a job for decades. The core `ContendedWriteError` covers the last one, recognised in core because no platform classifier could. Four properties are load-bearing. **The path is bounded, or a job would be immortal**: a deferral consumes no attempt, so a shop answering 503 for ever would recycle its jobs for ever - `queued`, holding that connection's lane scope slots, never `dead`. `sync_jobs.deferredTotalMs` accumulates the deferral wait GRANTED to the job, and once it would pass the budget (`OL_JOB_MAX_DEFERRED_WAIT_SECONDS`, 24 h) the job rejoins the ordinary retry ladder and can reach `dead`. A cumulative budget rather than a deferral count, because the grants differ by an order of magnitude (60 s for a throttle, 300 s for maintenance) and a count would mean a different amount of patience per reason. **A deferral never beats a terminal answer**: the deferral check runs only when `isNonRetryable` said no, because both registry methods OR across every registered classifier, so a cause one plugin calls deterministic and another reports as deferrable must die rather than defer for ever. **A deferred job is visible as deferred**, not merely as queued: the reason and the spent budget are written into `lastError`, and the jobs list and detail page say `deferred` beside the attempts counter that a deferral deliberately leaves standing still. **The duration recorded is honest about what ran**: a 429 or 503 means the handler executed and spent real time before the destination turned it away, so that path writes `lastAttemptDurationMs`, while the paths where nothing executed (a rate-limit slot that never opened, a job killed for having no registered handler) CLEAR the column rather than leaving an earlier attempt's number beside a new state.
...
- **First adapter (#1908)**: `@openlinker/integrations-eparagony`, adapterKey `eparagony.documents.v3`, platformType `eparagony`, registry capability `Fiscalization`. eparagony.pl is a private e-receipt *distribution hub* sitting in front of a fiscal printer driven by the vendor's own software, not a fiscalizer itself, so `EparagonyFiscalizationAdapter` implements `FiscalizationPort` + `FiscalRegistrationLocator` and no device sub-capability - `print`/`fiscalize` are booleans inside the vendor's document payload, not device operations, so there was never a device operation for one to call. #1910 was closed `not_planned` on this finding: the printer sits below the middleware boundary on every known adapter, so a device sub-capability has no shipped consumer. Three properties of that adapter are worth knowing because they are consequences of the neutral contract meeting a real provider, not local implementation detail. (1) **`registerTransaction` blocks on a bounded status poll.** The vendor's create answers `202 Accepted` before the device has registered anything and may still fail afterwards, so returning at `202` would report a completed registration that does not exist - and `registered` is TERMINAL in core. The call therefore polls the document status to a terminal value inside a wall-clock budget kept strictly under core's supported provider round-trip ceiling, so an expired in-flight lease can never be re-claimed mid-call (same shape as the inFakt async-task poll, #1763). (2) **The locator only works because the adapter mints its own document key.** The vendor's single document read is keyed by a path token and it publishes no search by order id, registration key or date range; its create accepts an OPTIONAL caller-supplied token, so the adapter derives one deterministically from `(connectionId, idempotencyKey)` and re-derives it to reconcile. Without that, an indeterminate call would leave nothing to look the registration up by - which is exactly the state ADR-042 decision 7 forbids resolving by a blind resend. It also turns the vendor's "document already exists" rejection into a resolvable outcome rather than a false `rejected`. (3) **A locate answers from three outcomes, and one contract mismatch is still carried honestly rather than papered over.** `locateByQuery` reports `registered` / `held` / `not-found` (ADR-042 amendment #2502, decision 1, #2520): a document the vendor holds at a non-terminal status answers `held`, which reconcile surfaces as `still-unknown` and which leaves the record byte-identical. **`still-unknown` means the check settled nothing, NOT that the provider is holding the sale** - an answer core could not read lands on the same outcome, so it is stamped `detail: 'unreadable-answer'` and neither the log line nor the published API description states a fact about a provider that no adapter supplied (#2583 review). Reconcile answers from a CLOSED four-value vocabulary - `resolved` / `not-found` / `unsupported` / `still-unknown`, of which only `resolved` writes - so a panel offering *check with the provider* can state every answer it may get (#2522); a failed CHECK is deliberately not one of them and raises `FiscalReconcileCheckFailedException` -> HTTP 502, because reporting a transient network failure as the structural `unsupported` would tell an operator to stop retrying something that is merely transient, and reporting it as `not-found` would assert an absence the provider never asserted, instead of the pre-#2520 `null` that reported normal in-provider processing to the operator as an absence. Only `CONFIRMED` may terminalise a record, a vendor-reported `ERROR` answers `not-found` - which means no registration exists for those coordinates, never that the provider holds nothing, since there it holds a document it reports as failed - and core normalises whatever a locator returns through `readFiscalLocateAnswer`, so an out-of-tree adapter still answering the pre-#2502 `FiscalLocateResult | null` shape keeps working. What remains carried is the vendor's fiscalization-status webhook, deliberately **not** wired, because `CanonicalInboundEvent.domain` is a closed union with no fiscalization member and no fiscalization job to route to, so a registered decoder would authenticate every delivery and then dead-letter it. Regime specifics - the PL rate-letter slots, grosze, the receipt envelope - live entirely adapter-side, and the per-line rate is resolved from what OL passes through or from an operator-declared per-connection slot, never inferred (ADR-042 decision 8; see the package README for the full gap list). **Whether a registration is running RIGHT NOW is readable (#2521, ADR-042 amendment #2502 decision 2).** The fact previously existed only as the 409 a concurrent attempt received - the correct answer to a write and useless to a reader, since a surface cannot tell *someone else is registering this right now*, which is reassuring and needs no action, from an error. `IFiscalRegistrationService.getInFlightRegistration` and its invoicing twin `IInvoiceService.getInFlightIssuance` answer it as a pure read over persisted state - no lock, no provider call, nothing attempted - in one shared neutral shape, `SalesDocumentInFlight` (`@openlinker/core/sales-documents`, which owns it for the same reason it owns the reason vocabularies: a per-order surface covering both kinds must not branch on which context answered). Three properties are load-bearing. It reads through the SAME predicate the write path claims against (`isLeaseLive`), so what an operator is shown and what a second attempt would hit cannot drift. An EXPIRED claim reads as not-in-flight, because an expired lease means the previous attempt died rather than that one is running, and saying otherwise would ask an operator to wait for work nobody is doing. The `since` it carries is the claim-holding record's last write and therefore a LOWER BOUND on elapsed time, never the attempt's start - nothing persists a claim-start instant, and on the invoicing path the numbering allocation writes the record inside the live lease - so a surface may say "running for at least 38s" and never "started at". And the lease EXPIRY is deliberately not carried: it bounds how long the claim blocks a same-key retry and says nothing about how long the provider will take, so a surface handed it would render a countdown that reads as a completion estimate OpenLinker cannot make - it hands the sale over and waits for one answer, observing no steps in between. Visibility only: the lease semantics, the exactly-once guarantee and the 409 are all unchanged. **What a registration produced is projected without what it produced (#2523).** `FiscalArtefactSummary` carries the three facts an affordance needs - the adapter-declared form, its disposition HINT and a label, plus the MIME type where the adapter knows one - and never the payload, so a per-order view can decide there is something to offer without carrying a customer-facing document into a list read; a caller that needs the bytes reads the registration record, whose DTO keeps them. An empty list stays a SUCCESSFUL registration and is kept distinct from `null`, which means the attempt never got far enough to produce anything - collapsing the two would report an unfinished attempt as a completed pure-reporting one. **No delivery claim is derivable from it, and that is a property of the TYPE rather than a convention**: `disposition` is what the adapter suggests a caller might do, never a record that it happened, and the summary carries no timestamp, recipient, status or attempt count a surface could read as one - no shipped adapter reports whether a document reached a buyer, so nothing may say it did. Status/link surfacing on the order is #1909 and the connection UI is #1911. Which document a given order gets - invoice or receipt - is the sibling routing decision ([ADR-041](./architecture/adrs/041-sales-document-routing-policy.md)), not this capability's.
...
- **Key types**: `SalesDocumentKind` (`CoreSalesDocumentKind | string`, i.e. open-world per ADR-041 decision 10, with `'invoice'` and `'fiscal-receipt'` the two kinds core recognizes structurally; a regime may declare a third, but a document PROPERTY such as "receipt carries the buyer's tax ID" still belongs on the `fiscal-receipt` outcome rather than becoming a kind of its own) and `SalesDocumentDecision` (#2155, the resolver's output — a `(documentKind, connectionId)` pair or `unresolved`). `SalesDocumentUnresolvedReason` (*routing could not decide*) and `SalesDocumentGateBlockReason` (*routing decided, or explicitly did not, and issuance is still not allowed*) are kept as **two** unions because they answer different questions, with `'unresolved-routing'` as the one bridge value carrying the routing reason alongside it (ADR-041 §107). Plus `SalesDocumentBlock`, the `SalesDocumentBlockOutcome` the gate reports (`none` / `blocked` / `indeterminate`), `SalesDocumentAttentionReasonValues` (the aggregate-worthy subset), and the two `is*` coercion guards.
- **Two pure resolvers, both live, rule-engine-first (#2173).** `evaluateSalesDocumentRules` (#2170, the country-agnostic 4-tier fallback evaluator — rule match → country default → `★` Rest of world → `unresolved`/`no-configuration-for-country`) is now consulted by `AutoIssueTriggerService` BEFORE `resolveSalesDocumentRouting` (#2155, the `operator-configured` single-primary model), via `ISalesDocumentRulesService.resolveRouting` (`SALES_DOCUMENT_RULES_SERVICE_TOKEN`, `@openlinker/core/sales-documents` — `InvoicingModule` now imports `SalesDocumentsModule`). A small pure mapper (`toSalesDocumentOrderFacts`, `libs/core/src/invoicing/application/mappers/`) builds the engine's `SalesDocumentOrderFacts` input from the order's DELIVERY (shipping) address country + `totals`, with `buyerHasTaxId` read from the order's own buyer tax id since #2599 (never defaulted to `false` - "unknown" must not collapse into "known to have no tax id", which is why `SalesDocumentOrderFacts.buyerHasTaxId` was widened from `boolean` to `boolean | undefined`). **Fallback precedence**: any rule-engine decision OTHER than `unresolved`/`'no-configuration-for-country'` (a `route`, an `aggregate`, or a different `unresolved` reason) is used as-is and flows into the SAME dispatch/reporting pipeline as before — a non-`no-configuration-for-country` unresolved reason therefore surfaces via the existing `SalesDocumentBlockOutcome` seam, never a silent fallback. Only `'no-configuration-for-country'` (or an order with no delivery-address country at all) falls through to `resolveSalesDocumentRouting`, which keeps an untouched install byte-identical to its pre-#2170 behaviour. The flagship Poland `buyerHasTaxId` condition can match a real order as of **#2599**, which carries the buyer tax id on the order contract in **three** states - absent (the source asserted nothing), `null` (the source asserted the buyer has none), or the id verbatim. Four properties are load-bearing. The three states never collapse: `buyerHasTaxId` is `undefined` for absent and `false` only for a positive "has none", so `evaluateSalesDocumentRules`'s existing handling of an absent fact (an `undefined` compares unequal to both `true` and `false`) still means "this order was not decided on a tax id". The value is carried **verbatim** - never validated, normalised or format-checked, because ADR-026 keeps national specifics in the provider adapter and a `NIP` rule in `libs/core` is exactly what that forbids. It is **PII-gated** like `customerEmail`: for a sole trader the tax id identifies a natural person, `sanitizeAddress` already drops it from the snapshot, and a hash-only deployment stores no scalar either, which reads back as "not asserted". And **only the fact is wired, not a refusal** - `order_records.buyerTaxId` is denormalized off the snapshot's billing address (`NULL` = not asserted, `''` = asserted-none, otherwise the id - so a bare `IS NOT NULL` reads the middle state wrong; use `decodeBuyerTaxIdColumn`), but no gate acts on it and `'missing-required-tax-id'` is still never written. PrestaShop is the only source that supplies it today, from `ps_address.vat_number`. Neither the Allegro nor the WooCommerce order-source adapter reads one - Allegro's own checkout-form invoice block carries a company tax id that OL's `AllegroCheckoutForm` type does not model, and WooCommerce's `billing` block has no tax field at all - so an order from either reads as not asserted, never as a buyer known to have none.
...
- **Zero outbound edges to sibling CORE contexts, enforced — narrower than "zero outbound edges" since #2170.** Both `invoicing` and `orders` value-import it — from a domain entity, a repository port, an application interface, an application service and an infrastructure repository. `libs/core/src/__tests__/barrel-purity.spec.ts` fails the build on any non-relative import of a `@openlinker/core/<ctx>` specifier under the directory (with three authorized type-only exceptions, all reaching a `<ctx>/types` cycle-breaker sub-barrel that re-exports no runtime value: `Order` from `@openlinker/core/orders/types` (#2155), and the invoice and fiscal status vocabularies from `@openlinker/core/invoicing/types` / `@openlinker/core/fiscalization/types` (#2515, ADR-065) - a type-only import erases, so none of the three emits a `require()`) — `@nestjs/*` / `typeorm` / `node:*` imports are unrestricted, since those are ordinary infrastructure dependencies every other context also has.
...
- **Five persisted states on `order_records`, and two predicates a consumer gets wrong.** The columns land with #2124 and are written by the stamp service and its jobs (#2125):
  ...
  A consumer must read `reportingCurrency IS NULL` as "unstamped" and **never** `exchangeRateId IS NULL`, which is also NULL on the same-currency path - i.e. on the overwhelming majority of orders, every one of which would be silently discarded. `fxStampedAt IS NULL` is not an equivalent test either: it is also NULL on a deferred row, which is legitimately still in flight rather than unstampable. The sweep predicate is `reportingCurrency IS NULL AND (fxStampedAt IS NULL OR fxStampedAt < now - terminalRetryDays)`: the first arm is exactly the never-attempted and deferred states, and the second is a **cooldown re-admission of a terminal row** (#2135 review). A terminal answer is terminal about the *classification*, not about the world - `no-rate-source` clears the moment a host is rewired with `FxIntegrationModule`, and an `unsupported-pair` raised by a throttled public provider clears by itself - so without the second arm a transient 429 permanently cost those orders their reported figure, silently and with no re-open path anywhere in the codebase. The cooldown defaults to 7 days (payload-overridable per scheduler descriptor), and a terminal re-answer *moves* `fxStampedAt` forward so the row is retried once per cooldown rather than on every tick. `reportingCurrency IS NULL` is present in **both** arms, which is what keeps the stamp itself immutable: a row that carries a figure is never re-entered whatever its marker says - **with exactly one exception, added by #2468 and recorded as an amendment to ADR-040**. An order stamped under a PREVIOUS reporting-currency era is not merely recorded in an old currency, it is invisible to every KPI: `getDailyOrderAggregates` sums `reportingTotalAmount` only where `reportingCurrency = :currentReportingCurrency`, so a prior-era stamp reads as unconverted, the same bucket as a row that was never stamped at all - and immutability is what keeps revenue under-reporting forever. `OrderRecordRepositoryPort.clearFxStampForRestatement` is therefore the one writer allowed to move a stamp: it nulls the same six columns the `1840000000000-reset-fx-stamp-for-mislabelled-prestashop-orders` migration established, in one guarded statement, and is reachable only from `OrderFxRestatementService` inside an `analytics_remediation_runs` audit row. **The ledger is what makes the exception acceptable rather than a hole in the rule**: what ADR-040 forbids is a financial figure that moves with no traceable cause, and a row naming who asked, when, and over how many orders is a different act. Two details are load-bearing. `fxIntendedCurrency` must be cleared with the rest - `resolveIntent` re-pins a persisted intent and never consults the settings service, so leaving it behind re-stamps the SAME stale currency and the repair looks successful while changing nothing. And the child stamp jobs use a run-scoped idempotency key (`fx:restate:{runId}:{orderId}`), because `OrderFxStampService.enqueueRetry` already spent the bare `fx:{orderId}` for any order that ever degraded to a retry and `sync_jobs.idempotencyKey` is globally unique with no TTL. This is deliberately NOT #2096, which remains open: that is re-expressing the whole corpus after a setting change and needs an era model, whereas this is a bounded, operator-initiated repair of a population the Data Coverage panel already reports, over a window the operator already chose. The same reasoning made 429 and 408 transient rather than terminal in both adapters (`isTransientFxStatus`), and made an empty provider registry a boot failure (`CurrencyModule.onApplicationBootstrap` -> `NoExchangeRateProvidersRegisteredError`) rather than a condition discovered one silently-unstamped order at a time. Because the equal-currency path writes `reportingTotalAmount = totals.total` with no lookup and no I/O, `reportingCurrency` - not `exchangeRateId` - is the only discriminator that means what it says. Any analytics figure built on stamped rows must therefore be paired with an unstamped count.
...
- **The registry is readable over HTTP (#2778).** `GET /currency/rates?from=&to=&date=` (`apps/api/src/currency/http/exchange-rates.controller.ts`) is a pure registry **read** of the stored row, or 404 - backed by `IExchangeRateLookupService.findRate`, a one-method `findByKey` passthrough deliberately kept separate from `ICurrencyRateService` so the endpoint structurally cannot reach the get-or-create `getRateFor` and spend the deployment's provider budget on an HTTP caller's behalf. Gated by the global `JwtAuthGuard` only, not `@Roles('admin')` - it is the evidence behind a figure any operator can already see on `/analytics`, not a deployment-wide write. The same provenance is threaded onto the display-currency conversion result: `AppliedRate {from, to, rate, rateDate, source, derivation, sourceRef}` on `NativeCurrencyBreakdown` / `OrderDateConversionResult`, `null` exactly when no lookup happened (an unresolved row, or a same-currency identity that never called the provider), surfaced as `appliedRates[]` on `DisplayCurrencyConversionDto`. `rate` stays a `string` end to end on both surfaces, matching the `numeric(18,8)` registry column.
...
### 20. Operational Settings

*See [ADR-069](./architecture/adrs/069-operator-settable-sweep-pacing.md) for the decision rationale.*

- **Responsibility**: hold the knobs that decide how hard OpenLinker works a shop's catalogue, and let an operator reach them (#2651). Before this, the catalogue sweep budget, the inventory sweep budget, the sweep page size and the deletion-audit budget and cadence were code constants with env-var overrides, so changing one meant editing a `.env` and restarting the worker - and since #2279 moved the scheduler into the worker, `OL_MASTER_PRODUCT_RECONCILE_CRON` set on the api was silently ignored, which is a second reason the env-var route is a poor operator surface.
- **Location**: `libs/core/src/operational-settings/` - a **leaf** context: nothing under it imports a sibling `@openlinker/core/*` context and nothing under it speaks HTTP. Interface: `GET`/`PUT /operational-settings` (`apps/api/src/operational-settings/`, `@Roles('admin')`). Storage: the singleton `operational_settings` row, the third instance of the shape `ai_provider_active_setting` (#451/#452) and `reporting_currency_setting` (ADR-040) established.
- **The env rung answers for THIS process, and the answer says so (#2660 review).** `OperationalSettingsService` runs in both the api and the worker, which load their own `.env` and carry separate `environment:` blocks in `docker-compose.yml` - and since #2279 the sweeps run in the WORKER. So an env rung resolved in the api describes the api, never the process that applies the value; an operator setting `OL_PRODUCT_SYNC_PAGE_LIMIT` in `apps/api/.env` (the file whose `.env.example` they read) saw the settings page report it while the worker kept the default for ever - reported ≠ enforced, on the exact axis #2651 exists to close. Every resolved value therefore carries `workerMayDiffer` (`source !== 'setting'`, since only the shared ROW is authoritative for both processes), the page states it once above the panels, and both `.env.example` files now say which process reads each variable. The rung is reported rather than dropped, because dropping it would answer `default` for a value an env var really did change. **The two sweep CADENCES are reported for the same reason**: `OL_PRODUCT_SYNC_CRON` / `OL_INVENTORY_SYNC_CRON` are settable and `apps/api/.env.example` ships one uncommented, so a calculator assuming the shipped 20 / 15 minutes reports a pass length an order of magnitude wrong on an install that changed one. They are read-only on this surface (no column, no input) and `CORE_CAPABILITY_TASKS` consumes the same constants as its `defaultCron`, so what is reported and what the scheduler registers cannot drift.
- **Resolution is `DB row -> env var -> code default`, and every value is reported with the rung that produced it** (`OperationalSettingsView`, `source: setting | env | default` - the `ReportingCurrencySettingsView.source` shape). Every column is nullable and `NULL` means "not set", so an install that upgrades past the migration and never opens the settings page behaves byte-identically to how it behaved before, and an install that had only ever set an env var keeps getting that value. That is what makes the change safe to deploy rather than a behaviour change dressed as a settings page. Reporting the rung is what lets a UI render `500 (default)` instead of comparing against a hardcoded number client-side - a client-side comparison is a second copy of the default, and it is wrong the day the default moves.
- **The worker reads the setting PER TICK, not at boot.** Both sweep handlers and the deletion-audit handler call `IOperationalSettingsService.resolve()` inside `execute()`, so a budget change reaches a running worker with no restart - a value that needed one would be barely better than the env var it replaces. The cost is one singleton primary-key lookup against a run that is about to issue platform calls, which is the same reason `MultiProviderAiCompletionAdapter` reads its active provider through the database on every completion. The property is **proven by a test rather than asserted**: a spec changes the budget between two `execute()` calls on the SAME handler instance and asserts the second run enqueued more children.
- **The cadence is the honest exception, and says so.** A `CronJob`'s expression is fixed at construction, so `SchedulerService` snapshots the resolved cadences in `refreshOperationalSettings()` - awaited by `SchedulerLeaseCoordinator` immediately before `start()` - and a cadence change applies at the next scheduler start (a lease hand-over or a restart). The response states `cadenceAppliesAt: 'next-scheduler-start'` rather than implying otherwise. That refresh **never throws**: a database hiccup while acquiring the lease must not stop the crons registering, since a fleet with no scheduler at all is strictly worse than one running on the previous cadence, so a failure leaves every task on its `cronEnvVar` / `defaultCron` resolution. Only `master-product-reconcile` declares a `cadenceFromSettings`; widening the surface to every task is a separate decision.
- **Two kinds of ceiling, deliberately not conflated.** `OPERATIONAL_SETTING_BOUNDS` carries a `recommendedMax` (advisory - OUR judgement, exceedable) and an `absoluteMax` (refused whatever the request says), each with a reason string the UI renders instead of inventing one. A write above the recommendation needs `acknowledgeAboveRecommended: true` on the request, so a mistyped number is still refused while a deliberate one is honoured; the flag is per-request and is stripped before persistence, because one decision to run past our advice must not silently license every later write. The table is read by the API validator, by the response's `bounds` block (a form derives its limits rather than restating them) and by the worker's own clamp - reported === enforced structurally, the #2229 rule applied to a settings surface. **The resolve-time clamp uses `absoluteMax`, never `recommendedMax`**: narrowing an acknowledged value back down would show 5000 on the settings page and run 2000 in the sweep, which is the reported-versus-enforced gap in its purest form. A DTO decorator is a convenience for the OpenAPI document and a fast rejection, never the gate (the browser is not a trust boundary and the raw JSON path bypasses any form), and its `@Max` is therefore the absolute ceiling - enforcing the recommendation there would refuse a legitimate acknowledged request before the service could weigh it.
- **A platform's own wall is enforced where the request is built, and it REFUSES rather than narrowing.** The first revision of this table capped `sweepPageSize` at 100 on the stated grounds that it was "PrestaShop's collection page size and WooCommerce's hard `per_page` cap". **Both halves were wrong for that value.** PrestaShop pages through the `limit=[offset,]count` comma syntax with no cap (`prestashop-query.builder.ts`; #2644 measured `limit=99000,500` against a live shop), so 100 was an OpenLinker convention presented as a platform fact - the kind of claim that quietly makes our opinion binding on somebody else's hardware. And WooCommerce's `per_page` cap, which IS real, does not apply to that value at all: it is the batch child's `groupSize`, never a `per_page`. The genuine `per_page` carrier is the enumeration page size, and `WooCommerceProductMasterAdapter` **refuses** an enumeration above it (`exceedsAdapterPageSize`) rather than clamping - a refusal being the only safe answer THERE, because `readPagedIds` infers end-of-collection from a page shorter than the size it requested, so a clamped page reads as the end of the catalogue: a cycle "completed" after 100 products, the cursor cleared, and the same first 100 were swept on every tick for ever behind one warn line (#2660 review). Before the clamp existed WP REST answered 400 and the job failed loudly, which is strictly better; a page size an operator set is never quietly not what was sent, and never silently truncates a sweep either. `sweepPageSize`'s absolute ceiling of 500 is a real mechanism rather than a round number: the batch child's ids are `|`-joined into a query-string filter, so 500 seven-digit ids is roughly 4 KB of query string, comfortably under the 8 KB request-line limit nginx and Apache default to, and 2000 would not be.
- **The deletion audit cannot be disabled here.** #2222 made `master.product.reconcile` the deletion authority, so switching it off silently reopens #1689 - a deleted product whose offers keep selling. `OperationalSettingsInput` therefore carries a cadence and no enablement, the view reports `deletionAuditAlwaysEnabled: true` so the absence reads as a decision rather than an oversight, and a cadence firing less often than weekly is refused as a disable in disguise (`0 0 1 1 *` is a valid cron expression and an effectively dead audit). **The check probes by TIME, not by occurrence** (#2660 review): measuring the gap between the next TWO firings accepted `0 0 1,2 1 *` - two firings one day apart, then nothing for a year - so the audit ran twice a year while the gate reported it closed. Walking N consecutive firings cannot be made sound at any fixed N either, because a day-of-month list fires 28 times in January and then not for eleven months, so the offending gap always sits just past N. Stepping forward one maximum-gap at a time across a year and asking "is there a firing within the next 7 days from HERE" tests the actual property at a cost independent of how densely the expression fires. `OL_MASTER_PRODUCT_RECONCILE_ENABLED` remains the only off switch. A stored cadence is also re-validated on the way OUT, falling back rather than handing `SchedulerService` an expression that makes `new CronJob` throw - that aborts the whole registration loop and leaves the fleet with no scheduled tasks at all.
- **The two per-sweep batch-size env vars survive as narrower overrides.** `OL_PRODUCT_SYNC_BATCH_SIZE` and `OL_INVENTORY_SYNC_BATCH_SIZE` are consulted only while no operator has set the shared page size (`sweepPageSize.source !== 'setting'`). Collapsing them into the one setting would silently change the inventory sweep's page size for an install that had tuned only the product one - a behaviour change on upgrade, which is what the fall-through design exists to avoid.
- **Explicitly out of scope: the outbound request rate limit.** `Connection.config.rateLimit` is per-connection, already validated by `ConnectionService.validateRateLimitConfig` and already edited by `RateLimitSection` on the connection form (#1810). Adding it here would give one value two homes.
...
The rule applies only to imports from the bare top-level barrel `@openlinker/core/<ctx>`. The four documented sub-barrel families (`/services`, `/orm-entities`, `/testing`, `/types`) are governed separately - the first three by their own ESLint rules, and the `/types` cycle-breakers by `scripts/check-types-sub-barrels.mjs`, which bounds what those files may re-export at the source. See `docs/engineering-standards.md § Import Aliases`.
...
graph LR
  ...
  orders --> fiscalization
  analytics-trust --> integrations
  analytics-trust --> sync
  analytics-trust --> orders
  analytics --> orders
  fiscalization --> invoicing
  fiscalization --> sales-documents
  fiscalization --> sync
  invoicing --> fiscalization
```

`identifier-mapping`, `integrations`, and `events` form the most-depended-upon "infrastructure spine" (each used by 5+ siblings). The `sync --> events` edge was removed in #2163: `SyncJobBulkRetryService` was the context's only consumer of `EVENT_PUBLISHER_TOKEN`, and deleting the write-only `events.sync.jobs` publish left `SyncModule` importing `EventsModule` to satisfy an injection that no longer existed. `users`, `webhooks`, and `mappings` have minimal outbound coupling. `operational-settings` (#2651) has none either, and no inbound core edge at all - both host apps consume it directly (the api for the admin surface, the worker for its sweep handlers and scheduler). `currency` has **none**: it is a leaf with a single inbound edge from `orders` (ADR-040 § Decision 7), which is why `ICurrencyRateService` takes an already-resolved rate date and source rather than reading `placedAt` back out of an order.

`sales-documents` (#2100) is the opposite extreme and deliberately so: **a sink with zero outbound edges to sibling CORE contexts**. As of #2170 it is no longer framework-free (it carries its own `SalesDocumentsModule`, an application service, and repositories backing the rule engine), but nothing in it injects `@openlinker/core/<sibling>` — which is what lets both `invoicing` and `orders` value-import it from a domain entity, a repository port and an application service without any risk of a CJS module-load cycle. That property is pinned by `libs/core/src/__tests__/barrel-purity.spec.ts`, which fails on any non-relative import of a `@openlinker/core/<ctx>` specifier added under the directory (with the three authorized type-only exceptions documented there - `@openlinker/core/orders/types`, `.../invoicing/types` and `.../fiscalization/types`, every one of them a `<ctx>/types` sub-barrel carrying no runtime value); `@nestjs/*` / `typeorm` / `node:*` imports are unrestricted. The concern now has an ordinary `sales-documents.tokens.ts` — the `<ctx>.tokens.ts` exception (`engineering-standards.md § Symbol DI Token Re-export Convention`) applied only while the concern had no DI bindings to discover, and ended with #2170.

The `orders --> fiscalization` edge is #2516's: the per-order sales-document projection (ADR-065) reads the fiscal-registration records through `IFiscalRegistrationService`, exactly as it reads the invoice records through `IInvoiceService`. It is one-way and cycle-free by the same reasoning `orders --> invoicing` already is: `FiscalizationModule` imports `InvoicingModule` and neither imports an orders module, so `OrdersModule` importing `FiscalizationModule` closes nothing. Note this is a *static* module edge, unlike `InvoiceService`'s and `AutoIssueTriggerService`'s lazy `ModuleRef` resolution of the same service - those two live INSIDE `invoicing`, where a constructor dependency would reverse the `fiscalization -> invoicing` direction; a caller in `orders` has no such constraint.

The `analytics --> orders` edge (#2468) is **type-only in both files that carry it**: the `analytics_remediation_runs` ledger reuses `CoverageResolutionStatus` / `CoverageResolutionStatusValues` from `@openlinker/core/orders` rather than minting a parallel lifecycle union, so the ledger and the detector that reports on it cannot drift. It is one-directional — nothing in `orders` imports `analytics`. That direction is deliberate rather than incidental: the tax-inclusion setting (#2461/#2469) is *read* by the orders net-sales SQL, and threading it in as a plain `boolean` parameter from the `apps/api` layer (exactly as `currentReportingCurrency` already is) is what keeps it so. An `orders --> analytics` edge for that one flag would have created a needless `analytics ↔ orders` cycle, and would also have made the setting cacheable somewhere — the opposite of the "the toggle changes the very next query" guarantee it exists to provide.
