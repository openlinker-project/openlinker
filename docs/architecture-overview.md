...
- **First provider**: **KSeF** (Polish national e-invoicing), `@openlinker/integrations-ksef`, adapterKey `ksef.publicapi.v2`, registry capability `Invoicing`, with the `RegulatoryTransmitter` sub-capability (the manifest registers only `['Invoicing']`; `RegulatoryTransmitter` is a guard-narrowed sub-capability of `InvoicingPort`). It issues FA(3) `VAT` + `KOR` documents and clears them through the async submit→poll→UPO model. See the [KSeF setup guide](./integrations/ksef/setup-guide.md). The reconciliation-first / test-env-first posture and the deferred items (batch pipeline, inbound, Art. 108g payment title) are documented there.
