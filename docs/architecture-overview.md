...
`returns` (#2327, § 22) is deliberately **not** a fourth, and as of #2328 it is further from the property rather than closer: it now carries **two** real outbound edges. `returns --> orders` (for the `RefundReason` vocabulary it reuses verbatim) is type-only off the `@openlinker/core/orders/types` cycle-breaker sub-barrel and so creates no module-graph edge; `returns --> identifier-mapping` is a real NestJS module edge, since `ReturnsService` resolves a source-native order id to an OL internal one through `IIdentifierMappingService` and `ReturnsModule` therefore imports `IdentifierMappingModule` (acyclic — the infrastructure spine depends on no domain context). Registering the context in `ZERO_SIBLING_EDGE_LEAVES` would be a false claim rather than a tidy one. It does, however, share the leaves' **root-barrel** posture for the same cycle-safety reason, and the one-way discipline matters just as much in its case: `orders` must never import `returns` back.
