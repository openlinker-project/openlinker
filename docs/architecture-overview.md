...
- **Not fiscalisation.** Registering a sale on a certified fiscal device or register is a different act with a different issuer, device dependency, legal basis and retry semantics; it is a separate capability, not an `InvoicingPort` document type - see § 15 below.
- **Tax rates are an input, never a computation (#2009).** A line's `taxRate` arrives from the ProductMaster together with the product; OpenLinker never computes, infers or defaults it, a tax engine belongs inside the master or the sales channel rather than being a port here, and a missing rate blocks issuance instead of falling back to a silent value. See the VAT-rate annex in [ADR-026](./architecture/adrs/026-country-agnostic-invoicing-domain.md); the contract work is #2054.

### 15. Fiscalisation (planned - no code yet, #1908)

*See [ADR-040](./architecture/adrs/040-fiscalisation-capability.md) for the decision rationale.*

- **Responsibility**: register a sale as a fiscal transaction with a certified provider and surface the result on the order. **OpenLinker never issues a fiscal receipt** - issuance is reserved to a homologated device or a certified software register, and OL stays on the feeding side of that line (#1906, art. 111 ust. 6b / art. 111b ust. 2 of the PL VAT act).
- **Planned location**: `libs/core/src/fiscalisation/` - a context of its own, deliberately **not** an extension of `InvoicingPort` (different issuer, device dependency, legal basis, retry semantics).
- **Planned capability**: `FiscalisationPort` (registry capability `Fiscalisation`), whose base contract carries the single invariant operation both published middlewares converged on - register a transaction, receive back what must be printed or what identifies the registration. Regime-specific behaviour is composed as [ADR-002](./architecture/adrs/002-capability-ports-with-sub-capabilities.md) sub-capabilities: a device/peripheral operator for fiscal-printer regimes such as Poland (#1910), with journal/audit export and fiscal corrections explicitly deferred.
- **Trust anchor stays in the adapter**: certified device / security module / certified software plus hash chain / remote authority endpoint are observed classes, not core types - the class is not stable per country, so nothing in the shared contract may assume a fiscal printer exists.
- **Exactly-once registration is a core-owned contract guarantee** (caller-supplied idempotency key plus a durable per-connection unique index), because a double fiscal registration is a legal event for the seller. Amounts are transmitted as they stand: the VAT rate arrives from the ProductMaster and is never recomputed here.
- **First adapter**: eparagony.pl (#1908, PL e-receipts), with status/link surfacing on the order in #1909 and the connection UI in #1911. Which document a given order gets - invoice or receipt - is the sibling routing decision ([ADR-039](./architecture/adrs/039-sales-document-routing-policy.md), #2051), not this capability's.
...
- **FiscalisationPort** (planned, #1908): registers a sale with a certified fiscal provider; base contract is one operation (register a transaction, receive back what must be printed), with the device/peripheral half as a sub-capability (#1910). Designed but not yet implemented - see § Core Bounded Contexts, *15. Fiscalisation*.
