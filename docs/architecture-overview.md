...
- **The operator-facing read surface (#2501 / #2503, [ADR-065](./architecture/adrs/065-sales-document-read-surface.md) and [ADR-066](./architecture/adrs/066-sales-document-market-discovery.md); proposed, no code yet)**: three surfaces answer the same question about one order - `/settings/sales-documents`, the `/orders` row and the order-detail panel - and they read **one** neutral per-order projection rather than each composing the invoice and fiscal records themselves. The projection carries the resolved `SalesDocumentKind`, the status on the axis belonging to **that kind**, the identity fields a surface renders, the persisted block/unresolved reasons verbatim, and the other connections holding a record for the order. Two properties are load-bearing rather than stylistic. **The axes are per kind, and the other one is unrepresentable, not merely unset**: an invoice carries `InvoiceStatus` *and* `RegulatoryStatus`, a fiscal receipt carries `FiscalRegistrationStatus` alone and terminalises at `registered` with no authority rung at all - so a flattened single enum was rejected, because it makes *issued, then rejected by the authority* (the state `RegulatoryResubmitter` exists for) impossible to display, and puts a meaningless `cleared` on every receipt. **And a surface never re-derives a fact the backend persists**: the reason an order has no document is read from `order_records.salesDocumentBlockReason` / `salesDocumentUnresolvedReason` (#2100), never inferred from the order - a design draft that inferred it printed `no rule for PL` over an entirely different persisted cause, which is a false statement about the operator's own configuration. The list read is **batched** across order ids (the `getEarliestOrderDateByConnection` precedent, #2083), because the alternative is two reads per row. A companion **market-discovery** read enumerates the billing countries orders actually arrive from, with a count per window, so an unconfigured market is visible before an operator discovers it through missing documents - it is strictly a read, and creates no rule, default or routing of any kind. Starter templates are country-keyed data with a citable source and **Poland is the only entry**; a market without one is offered no recommendation rather than a generated one, and a detected-but-unconfigured market is a **neutral** state, since a fresh install with no routing is not broken. What the routing-first UX commits every surface to - state the decision, offer only that document, never fall back to asking - is recorded as an amendment to [ADR-041](./architecture/adrs/041-sales-document-routing-policy.md), and the four fiscal-lifecycle gaps the surfaces exposed (a third locate outcome, a readable in-flight lease, a closed reconcile vocabulary, asynchronous manual registration) as an amendment to [ADR-042](./architecture/adrs/042-fiscalization-capability.md). The UX these decisions were derived from is committed at [`docs/plans/mockups/sales-document-routing.html`](./plans/mockups/sales-document-routing.html), hash-addressable per state. Implementation is the #2513 epic; nothing in this bullet ships yet.
