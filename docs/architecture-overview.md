...
- **An adjustment can say WHY, and can refuse to happen twice (#2368, design § 7.3)**: `InventoryAdjustment` gains an optional `idempotencyKey` and a narrowed, closed `reason` (`return_restock` | `manual_correction`), and `adjustInventory` returns an `InventoryAdjustmentResult` — the same `Inventory` plus an OPTIONAL `adjustmentOutcome` reporting `disposition` (`applied` | `deduplicated`), `idempotency` (`honoured` | `unsupported` | `not_requested`) and the master's own `appliedAt`. It exists because #2370 restocks returned units THERE, in the master, since `inventory_items` is a mirror rewritten by every master sync — so an OL-side increment silently vanishes at the next tick — and a retried restock against a master with no conditional stock write would double-increment while the master's audit showed an unexplained delta. Four properties are load-bearing. (1) **Every added member is OPTIONAL, so the amendment is a source-compatible widening**: an existing implementer declaring `Promise<Inventory>` still satisfies the signature and an existing caller reading `Inventory` fields is untouched — the discipline `listExternalIdsByConnection`'s page argument was added under (#2219), and the reason a required key was rejected outright. The ONE compatibility cost is narrowing the pre-existing `reason?: string`, which was written by nobody and read by nobody in this tree; it lands on CALLERS, never on implementers (a narrower type is still assignable to a string read), and an audit value any caller may spell freely cannot be attributed or aggregated, which is the whole point of carrying it. (2) **An absent `adjustmentOutcome` means "not reported" and a caller MUST read it as `idempotency: 'unsupported'`** — a pre-#2368 adapter reports nothing, and reading that as a honoured dedupe would let a retry silently skip a restock that never happened. (3) **`appliedAt` is the MASTER's instant, never `new Date()`** — the change happened in another system and OL's clock is not a witness to it (the #2336 `declinedAt` rule, restated by #2367's custody transitions); `null` is an honest absence rather than "now". WooCommerce reads `date_modified_gmt` only, since `date_modified` is site-local with no offset. (4) **An adapter that CANNOT dedupe says so rather than pretending.** WooCommerce honours a key by remembering it in the shared `CachePort` for seven days (matching the `jobdedup:*` TTL, so the retry ladder's six-hour backoff cannot outlive the window), recorded only AFTER the write succeeded — recording first would suppress the retry of an adjustment that never landed, the exact failure inverted — and a dedupe hit re-reads and returns the master's CURRENT stock rather than replaying a stale figure. The window is deliberately bounded and honest about its bound: `CachePort` exposes no atomic set-if-absent, so two genuinely concurrent same-key calls can both apply; the threat this closes is the SEQUENTIAL job retry, and #2370 mints one deterministic key per disposition sequence (`return:{returnId}:{lineId}:{seq}`, never wall-clock) so two operators never share one. A host wired without a cache applies the adjustment and reports `'unsupported'` — never `'honoured'`, because a caller that believes a key was honoured stops defending against the double-increment itself. WC carries no stock-reason field, so the reason reaches the operator's log rather than the master's audit trail. PrestaShop still refuses `adjustInventory` outright, which is #2369's subject.
...
- **Custody is driven, and the rules are pure (#2367, `W2-30`)**: the five-value custody machine gets its transitions in `domain/domain-services/return-custody-transitions.domain-service.ts` — `advanceReturnCustodyToInTransit` / `applyReturnCustodyReceipt` / `applyReturnCustodyDisposition` / `markReturnCustodyNotReturned`, plus `isReturnCustodyFinished` (the custody half of the list's `All open` segment). **Pure functions, not a service**, the shape every other shipped rule engine in this tree takes (`applyPricingRule`, `checkRequiredToSell`, `resolveSalesDocumentRouting`, `deriveOrderLifecyclePhase`): there is nothing to inject, and #2370 computes its custody move INSIDE the transaction that already holds the row, so a service method would have invited a second read and with it a read-then-write race. They decide the resulting state, `receivedAt` / `disposedAt` and the counters the move implies; they **persist nothing** — #2370 (`W2-33`) owns the write, `restock_blocked` and the master `adjustInventory` call. **Transitions are per line and quantity-aware**, which is the whole reason custody lives on the line: a return received in two parcels legitimately holds `received` on one line and `disposed` on another, and `received` is re-entrant because the COUNTERS express partial receipt (adding a state to say what a counter already says is the double-encoding spec § 3.1 rejected). Three rules are decisions rather than implementation detail. **`in_transit` takes the SOURCE's instant, never OL's** — it is a claim about the outside world (a buyer handed a parcel to a carrier) that OL cannot witness, the same rule #2336 fixed `declinedAt` under, so a 2xx can never stand in for a channel-reported fact; the other three states are entered by an operator acting inside OL, where OL's clock IS the authority. **`not_returned` is always an operator act and never a timeout or a sweep** (spec § 5.2) — which also settles the question `ReturnRepository.warnOnVanishedLines` left open: a line the source stops reporting is not auto-transitioned, because a source withdrawing a line says nothing about physical custody. And **`not_returned` is refused on a partially received line** rather than approximated: custody is single-valued per line, the line still holds goods needing disposition, and there is no `quantityNotReturned` counter for the shortfall to move into — such a line finishes on the received branch with the shortfall permanently visible as `quantityAdvised - quantityReceived`, and giving it a state of its own is a model change with its own migration and spec amendment. Every refusal is one domain error (`ReturnCustodyTransitionError`) carrying a CLOSED reason union, so #2370's actionable code and #2376's 409 branch on a value rather than on a message string. Every switch closes with `assertNever`, which is what gives spec § 3.1's `inspected` reversal gate teeth: re-admitting the value makes each consumer a compile error rather than a silent fallthrough. **No migration** — the union shipped in its final five-member shape in Wave 1c.
- **Custody is WRITTEN, and a line's history is a set of ACTS beside the counters (#2370, `W2-33`)**: `IReturnCustodyService` gives the aggregate its two operator write paths — `receiveLine` and `disposeLine` — plus `markStockHandledManually`, and is the tree's first production caller of `InventoryMasterPort.adjustInventory`. Alongside them lands an **append-only per-line act ledger** (`return_line_events`, migration `1852000000000`), written in the SAME transaction as the counters it explains. **A counter cannot key an idempotent trigger firing**: #2360 needs a three-parcel return to fire `return.received` three times, and `quantityReceived` going `1 -> 2 -> 3` carries no per-arrival identity and is indistinguishable from a correction; it is also what finally gives #2368's `return:{returnId}:{lineId}:{seq}` key a `seq` to name, and what lets `restock_blocked` be a property of ONE disposition attempt rather than of a whole line (a line may hold an applied restock and a refused one). The ledger is history BESIDE the invariant, never instead of it — the counters stay the `CHK_return_lines_quantity_ordering`-guarded columns and are never recomputed from the acts at read time, which would move the invariant out of the constraint's reach. Five properties are load-bearing. (1) **A blocked restock does NOT increment `quantityRestocked`.** The ACT is the disposition and is never rolled back (the goods really were disposed of), but the COUNTER records *book-confirmed* restock and a refused book write confirmed nothing — so the units stay in `quantityReceived` until an operator attests (spec § 5.4), which is what lets #2381 assert that no surface renders blocked units as restocked. `applyReturnCustodyDisposition` is therefore called only AFTER the master answers and never on the blocked branch, which is why #2367 needed no amendment. (2) **The counter check runs BEFORE the boundary crossing** — the pure transition is called twice, once as validation (outcome discarded) and once at settle time against the locked row — because the other ordering lets an over-disposition increment the master's book and be refused afterwards, leaving the book and the counters disagreeing with no act explaining it. (3) **The act is persisted BEFORE the adapter call** (ADR-056's attempted-predicate ordering), so a process dying mid-call leaves `in_doubt` rather than silence; `in_doubt` never auto-retries, since OL does not know whether the units landed and guessing moves real stock, and its remediation is the same attestation a block gets. (4) **Disposal is serialized by a per-line `SyncLockPort` lock** (`return:line:{lineId}`) and every counter write takes `SELECT … FOR UPDATE` — two different guards for two different failures. The lock covers read-then-act across the provider boundary (the `invoiceIssueLockKey` shape, ADR-041 §3a: two concurrent disposals otherwise both pass the check and both apply under different `seq` values, so no adapter can dedupe them); the row lock covers the lost update the CHECK is **silent** on, since receiving 2 twice against `advised: 5` and recording 2 is perfectly legal. (5) **Every refusal is a named block, never a silent success** — `no-inventory-master` / `ambiguous-inventory-master` (never a silent pick: a wrong book is real stock in the wrong place) / `unresolved-product` / `ambiguous-product` / `master-refused` / `master-product-not-found` / `adapter-unresolved` / `unknown`, each carrying the adapter's own sentence verbatim because #2369 raises four distinct typed refusals that are four different operator actions and a closed union alone can only say "it refused" (#2231's rule). The restock target is resolved **by sku through `IProductsService`** — a return line carries no product id and cannot (`resolvedOrderLineId` is a by-value reference into the order snapshot's jsonb and nothing populates it yet) — which is the one new module edge, `returns -> products`, acyclic. The attestation is addressed **by LINE** (matching #2376's route and spec § 5.4's per-line action), settles every outstanding `blocked`/`in_doubt` act, records `restockedBy: 'operator_out_of_band'`, and **never writes stock or claims OL did**; the timeline keeps both entries permanently and only the *attention* clears. **No marketplace write originates here** — the ordinary `inventory.propagateToMarketplaces` fan-out carries the master adjustment onward, asserted by a spec. **Known limitation, named rather than approximated**: `not_returned` on a *partially* received line stays refused (#2367's open question, settled here) — the shortfall needs a COUNTER (`quantityNotReturned` + a widened CHECK), which an acts ledger does not supply, so spec § 5.2's *"Mark remainder not returned"* is unimplementable for such a line and the shortfall stays visible as `quantityAdvised - quantityReceived`.
...
- **The authorize WRITE, the orphan MATCH and the operator-authored CREATE (#2372, `W2-35`)**: three writes that share one theme — OL acting where no source has an opinion. **`return.authorize` is restricted to `origin: 'operator_authored'`**, which IS the slice: ADR-060's asymmetry says OL declines where the platform allows it (#2333) but authorizes only what OL itself authored, so a `source_ingested` return is refused with a named error (`ReturnAuthorizeRefusedError`, closed reason `'source-ingested'`) rather than a silent no-op — an operator clicking Authorize on a marketplace return must learn why nothing happened. It reuses the ADR-044 `order_changes` table (#2333) with a second `kind`, **never a second proposal mechanism** (asserted by `libs/core/src/returns/__tests__/no-second-proposal-mechanism.spec.ts`, which also bans a returns-side import of `OrderChangeRepositoryPort` — a second route into the same table would bypass the service owning TTL expiry). Four properties are decisions. It crosses **no adapter boundary at all** — for a return OL authored there is no source to ask, so the row is the AUDIT RECORD of the operator's act rather than a request awaiting an answer, opened and confirmed in one breath. `authorizedAt` therefore takes **OL's own clock**, which is the correct side of the #2336 / #2367 / #2371 rule: that rule forbids OL's clock standing in for a CHANNEL-reported fact, and this is OL's own act with OL as the sensor (the `ReturnLineEvent.occurredAt` side, opposite `claimDeclinedAt`). **A reused open proposal does NOT abort**, deliberately unlike decline: the ADR-044 slot exists to stop a duplicate REMOTE request and there is none, so refusing would let one crash between `openOrReuse` and `confirm` wedge the return behind a full TTL for no safety gain — the at-most-once guarantee is `claimAuthorizedAt`'s single conditional `IS NULL` UPDATE, exactly as `claimDeclinedAt` is. And there is **no guard on `declinedAt` / `closedAt`**: the four timestamps are four independent facts and none excludes another, so a guard would quietly reinstate the status ladder the model refuses. `'authorize'` joins `ReturnDownstreamTriggerValues` so the orphan refusal comes from the ONE seam and raises the same class every other trigger does.

  **`matchOrphanToOrder` is the operator's way out of the #2332 bucket**, beside the background reconcile, and unblocks every downstream trigger by construction — `assertAttributedForTrigger` re-reads the row, so filling `internalOrderId` is the whole of what a refund, a restock or a correction needs and no consumer changes. The order's existence is proved through **`identifier_mappings`, not through `orders`**: an internal order id exists there iff OL minted it while ingesting, which is a sound existence proof reached through a dependency this context already injects — and it costs no `OrdersModule` edge, which `ReturnsModule`'s docblock warns against manufacturing. It proves an id was MINTED, not that an `order_records` row still stands, the same by-value posture `internalOrderId` already has. The lookup is **unscoped by connection on purpose**: an order ingested under a DIFFERENT connection is one of the documented orphan causes, so scoping to the return's own `sourceConnectionId` would refuse precisely the case the action exists for. The write is `claimAttribution`, so **attribution stays monotonic and there is no unmatch** — an operator can fill the value in and never change one, which makes a mis-match uncorrectable in the product and is why #2376 must render a confirmation rather than a bare button; a lost claim race reports `'already-attributed'`, never a failure. The act is recorded on two additive nullable columns (`returns.matchedAt` / `matchedByUserId`, migration `1860000000000`) rather than in `return_line_events`, because that ledger is per-LINE and its rows sum back to the counters while a header-level attribution has neither a line nor a quantity; NULL means "not matched by an operator" and is **not backfilled**, since a return attributed at ingestion or by the reconcile had no operator act to record. **Line resolution is deliberately not re-run** — nothing in the tree populates `ReturnLine.resolvedOrderLineId`, so there is no resolution to re-run and inventing one would be a second, undesigned mapping.

  **`recordReturn` opens a return in OL against an order OL already knows**, for a source with no returns surface — the first production caller of `ReturnRepositoryPort.create`, which has had none since #2327. It writes `origin: 'operator_authored'` and **`externalReturnId: null`, never synthesised**: `UQ_returns_source_external` is partial precisely so such a row writes NULL, and a fabricated id would make the row claim a source it has not got (a source ADAPTER minting a deterministic key for its own platform, as Erli does, is a different and established thing). `sourceConnectionId` is REQUIRED and VALIDATED rather than derived — the column is `uuid NOT NULL` so the row must name a channel — and the mapping on that connection is what supplies `externalOrderId`, so the value is a fact OL already holds rather than an operator-typed string. `openedAt` takes OL's clock; **`authorizedAt` stays NULL**, because recording and authorizing are two acts, which is the entire premise of "authorization is an ACTION, not a state" and what leaves `return.authorize` a job to do. Lines land at the schema defaults, which is literally what makes an operator-authored return participate in custody and money exactly like an ingested one. **Calling it twice creates two returns, and that is accepted**: there is no natural key to dedup on, an operator may legitimately open two returns against one order, and core will not synthesise one to prevent it — a caller wanting at-most-once owns that above the seam.

  **`REFUND_ATTEMPTABLE_MONEY_STATES` deliberately does NOT narrow here.** #2371 records that the set contains `not_refundable` only because it is #2327's column default, and that a slice setting `pending` at creation narrows it to `pending | denied` with no call-site change. This slice declines that step: narrowing is sound only once EVERY writer sets `pending`, and source-ingested lines — every existing row in every install — would still carry `not_refundable`, making them permanently unrefundable, a silent money defect discovered one refund at a time. Setting `pending` for operator-authored lines *only* buys nothing (`isRefundAttemptable` treats the two identically) and costs consistency, while sharing the default is the literal reading of "participates in money exactly like an ingested one". The narrowing needs three things in ONE commit: ingestion writing `pending` on insert, a backfill migration for existing rows, and only then the one-line edit to the set.

- **The credit-note correction PROPOSAL, and the positional ambiguity it refuses to resolve (#2374, `W2-38`)**: a disposed return line PROPOSES an invoice correction, and the proposal is **data** — `ReturnCorrectionProposalService` reads, classifies and records; it resolves no adapter, references neither `issueCorrection` nor `CorrectionIssuer` (asserted by grep in `__tests__/proposal-never-issues.spec.ts`), and holding a proposal confers no authority to issue one. Issuing stays the operator's confirmed act through the existing correction flow, because a correction transmitted to a tax authority cannot be withdrawn. **The ambiguity is the whole subject.** `CorrectionLine.originalLineNumber` is a 1-based ARRAY POSITION, `InvoiceLine` carries no identifier and no sku, and `ReturnLine` carries no price — so `name` is the only axis the two shapes share, and an order that repeats one offer across two lines (the shape SPIKE-2375 found for Allegro commission refunds, where a claim keys on an order LINE ITEM) produces two identically-named invoice lines that no data in either record can tell apart. `ambiguous` is therefore a first-class outcome that lists EVERY candidate and selects none — never collapsed into `matched` even when every candidate carries the same price and rate, since picking one on an amount coincidence stamps a specific line number into a fiscal document. `candidatesPriceOrRateDiffer` reports that coincidence as evidence for the operator instead. Five properties are decisions. (1) **The `targetRef` is `correction:{returnId}:{invoiceRecordId}`**, because `UQ_order_changes_open_target` is `(internalOrderId, targetRef)` and carries **no `kind`**: the bare `returnId` namespace is taken by `return.decline` / `return.authorize`, while keying on the invoice id alone would collide across RETURNS — an order legitimately produces several (partial returns arriving in waves), and the second build would terminalise a proposal the first return's operator was mid-review on. (2) **The disposed quantity comes from the CHECK-guarded COUNTERS**, never a sum over the act ledger — so a #2370 blocked restock, whose units never reached `quantityRestocked`, contributes nothing; that line is excluded LOUDLY as `no-match` / `disposition-not-confirmed` (read once per build from `findOutstandingRestockEventsForReturn`) rather than vanishing, because crediting a buyer for units OL could not confirm came back is exactly the guess this programme refuses. (3) **The delta is quantity only** — a return does not change a unit price, so core computes no net and rounds nothing (ADR-063). (4) **A pre-#1297 document is REFUSED**, never diffed against the order's current state, which is precisely the defect the issued-line snapshot exists to prevent; and the target is the latest **`issued`** record, which makes #1297's correction-of-a-correction rule true by construction rather than by a special case, since a successful correction is itself an `InvoiceRecord`. (5) **Every non-proposing exit is a named outcome** (`no-invoice` / `no-line-snapshot` / `no-disposed-lines` / `nothing-correctable`), and `nothing-correctable` still returns the full proposal body so the operator reads every exclusion reason while no ADR-044 slot is held for a proposal with nothing to confirm. The persisted row always matches the answer — an identical open row is reused, a diverged one is `abandon`ed and replaced (comparison is over a key-sorted canonical form, because Postgres normalises **jsonb** key order and a plain `JSON.stringify` would make the reuse branch unreachable) — and it holds **no operator picks**: a pick belongs to the confirm request, or the next build destroys it. Names are matched exact-after-normalisation (trim, collapse whitespace, case-fold) and deliberately **not** diacritic-folded, unlike `DestinationCategory.searchText`: both names descend from the same catalogue, so folding buys nothing and could collapse two distinct products into a manufactured ambiguity. No migration — `order_changes.kind` is `varchar` with no enum and no CHECK. The HTTP surface is #2376 and the review UI #2382.
- **The write API, and the two statuses that carry meaning (#2376, `W2-39`)**: every Wave-2 returns write gets an HTTP surface on a new `ReturnWritesController` — the three per-line custody acts, `authorize`, `match-order`, the operator-authored `record`, the refund confirmation, and a `GET`/`POST` correction-proposal pair. Guarded by the existing `@Roles('admin','operator')` model with **no new permission value**, and the actor is always the verified token's, never the body's. Five things are decisions rather than plumbing. (1) **A blocked restock is a 2xx**, carrying `quantity`, `sku` and `connectionName` verbatim: the disposition succeeded and is recorded, only the master write failed, and the remediation copy names all three so a UI that had to fetch them separately would render the alarm a beat late. (2) **A counter-invariant violation is a 409 carrying an actionable `reason`** — the closed `ReturnCustodyTransitionError.reason` is emitted as a FIELD, exactly as `ReturnNotAttributedError.trigger` already is, because a client that parsed the message would break on the first reword. `non-positive-quantity` is 409 too despite reading like a validation fault: the DTO's `@IsInt() @Min(1)` catches every malformed request first, so reaching the domain check means the value was well formed and the STATE refused. (3) **`ReturnsExceptionFilter` grew from four mappings to fourteen**, because an unmapped domain error is a 500 for a state the service raised deliberately — the failure that filter exists to prevent. They follow one stated rule (404 = the addressed resource is absent; 409 = it exists and its state refuses; 400 = the payload was inapplicable), which is what SPLITS `ReturnMatchRefusedError` by reason: `already-attributed` is the return's own state (409), while `unknown-order` means the return is fine and the operator's order id names nothing OL minted (400) — answering 409 for both would tell an operator the return was in a bad state when their typo was the problem. (4) **The refund route performs the second write itself and never rolls back.** `ReturnRefundService` reports an intent (#2371's report-don't-persist seam, which is what keeps `OrdersModule` out of core's `ReturnsModule`); the controller writes the linked `RefundRecord` through `IOrderRefundService` under a per-attempt idempotency key derived from the claimed line ids. The two writes are not atomic and cannot be — two contexts, two repositories — so a failed record write answers **2xx** with `refundRecordWritten: false` rather than a 5xx: the money state already settled durably, reporting failure would send the operator to retry, and the retry would answer 409 `already-attempted`. `moneyMoved` and `refundRecordWritten` are separate booleans precisely so "nothing to write" and "money moved, record pending" are not both a null id. (5) **`GET /returns/:id/correction-proposal` cannot write**, so #2374's service gained `previewProposal`: both entry points delegate to one private `compute`, which makes "the preview and the recorded proposal agree" structural and "the GET persists nothing" true by construction. Binding the GET to `buildProposal` would have opened, abandoned and replaced ADR-044 rows on every page load. **`POST /returns/:id/commission-refund/claim` is deliberately absent** — `W2-37` (#2379) has not landed and SPIKE-2375 leaves the shape undecided, so a 501 placeholder would advertise a capability OL has not got and an invented attestation contract would settle at the HTTP layer a question the spike exists to answer.
- **The derived operator STAGE, implemented twice and pinned (#2377, `W2-40`, spec § 3.2)**: the returns list renders one glanceable stage per row — `Awaiting parcel` / `Partially received` / `Received — awaiting disposition` / `Disposed` / `Not returned` / `Declined` — computed purely from counters and timestamps. **It is a presentation projection and never a persisted column**; an int-spec asserts no `stage` column exists on `returns` or `return_lines`, because the cheapest way for one to arrive is a "while we're here" edit to a migration about something else, and persisting it would be a model change needing its own ADR. Six properties are decisions. (1) **The array order IS the ordinal** — first match wins and the SQL `CASE` is BUILT by iterating `ReturnStageValues`, so a reorder changes behaviour and `scripts/check-return-stage-mirror.mjs` treats one as a hard failure (the #2311 `OrderLifecyclePhaseValues` rule). Spec § 3.2 lists the six in *narrative* order, which is reading order for an operator and deliberately not encoded anywhere — one order, one rule. (2) **`partially_received` outranks `disposed`**: a return with 2 of 5 units arrived and both disposed is not finished, and calling it `Disposed` would say it is closed when three units may still turn up. (3) **`advised` in every arm means STILL EXPECTED, not originally announced.** `markReturnCustodyNotReturned` refuses a partially-received line, so a `not_returned` line always carries `received = 0` while its `quantityAdvised` would otherwise sit in the denominator forever — a two-line return with one line disposed and one written off would render `Partially received` permanently, a false statement about work the operator has finished, which the `not_returned` arm cannot rescue because that arm needs EVERY line written off. `notReturnedQuantityAdvised` is subtracted; the semantic shift is the rule, not a patch on the arithmetic. (4) **The FE derives it; the API does NOT ship a `stage` field.** The row already holds the counters, so asking the server for a string it can compute is a round trip for nothing — and a `stage` on the wire would leave nothing to mirror, which is the point. The list projection therefore gained a per-return counter rollup (`ReturnCountersDto`), aggregated in SQL because a header-shaped read must not hydrate every line of every row to compute six integers; the DETAIL read leaves `ReturnRecord.counters` **`null`** and folds its real lines instead, and `null` means "this read did not load counters", never "all zero". (5) **The mirror script pins structure, never semantics** — two rules of different strength, value+order equality core↔FE and structural-only core↔`RETURN_STAGE_PREDICATES` (same keys, same order, still consumed by a `ReturnStageValues.map(`). Its docblock forbids strengthening rule B, and the temptation is sharper than in the order-lifecycle original: that one justifies its limit by pointing at three `FALSE` placeholder arms, whereas every return-stage arm is semantically real, so a later reader will conclude a semantic comparison is now achievable. It is not — a TS function over numbers and a SQL `CASE` over aggregate columns admit no textual equality. **Meaning is proved by `RETURN_STAGE_FIXTURES`**, one table the core unit spec and the SQL int-spec both consume. (6) **Two count sets, two scopes**, and getting it backwards is invisible until an operator clicks a chip: bucket counts have `bucket` removed and `stage` applied; stage counts have `stage` removed and `bucket` applied — and `countReturnsByStage` strips `stage` itself, so the rule survives a caller that forgets it. `resolveScopedTotal` deliberately gains **no** stage arm: the bucket counts already carry the stage filter, so it resolves the stage-filtered total unchanged, whereas reading `stageCounts` there would report the stage-less total on a stage-filtered page. Spec § 4.1's six overlapping **segments** are a different, non-partitioning set and are out of scope here. `ReturnStatusCell` — which could render only `Declined`, for want of counters, and whose own docblock named this issue as its successor — is replaced rather than supplemented.
- **The worklist strip, the filter set and the two rails (#2378, `W2-41`, spec § 4/§ 5)**: Wave 1c shipped the list and the detail shell; #2378 gives them the state they were built to show. Six properties are decisions. (1) **Segments OVERLAP; stages PARTITION, and the two must not share a mechanism.** `ReturnStageCounts` (#2377) is a partition whose int-spec asserts `Σ byStage === total`; `ReturnSegmentCounts` is the opposite shape — one return can be `needs_disposition` **and** `money_pending` **and** `orphans` at once, and `all_open` deliberately overlaps almost everything — so there is **no sum assertion**, the warning saying so lives on the TYPE rather than only in a plan (the sibling asserts the opposite one file over, and a reader copying it is the likely failure), and the int-spec pins the INEQUALITY. `SEGMENT_PREDICATES` is therefore independent booleans, never a `CASE` ladder. (2) **ONE `segment` URL param with six values — a deliberate deviation from spec § 4.3's literal param list**, because that list cannot express three of its own segments: `Needs receiving` spans `awaiting_parcel` **or** `partially_received`, `Needs disposition` spans `received_awaiting_disposition` **or** a partially-received line with undisposed units, and `Money pending` means `pending` **or** `in_doubt`, while `stage` and `money` are each single-valued. `attention` and `orphan` are dropped as separate params — they ARE segments, and two spellings of one filter is drift by construction. (3) **`orphans` is an ordinary segment predicate, NOT a translation of `bucket=orphan`.** An earlier design mapped it at the API boundary; composed with the strip rule that would be a self-scoping bug — the FE would send `bucket`, leaving no `segment` to strip, so the orphans card would report the count of the scope it is already in. It reaches the SAME rule `ReturnRecord.isOrphan()` states through one shared `ORPHAN_PREDICATE` SQL constant (consumed by the bucket count, the bucket filter arm and the segment alike), and `bucket` stays an independently-usable filter a segment click never writes. (4) **The counts-scoping rule generalises**: bucket counts have `bucket` removed and **every other filter applied**; stage counts have `stage` removed; segment counts have `segment` removed — and `countReturnsBySegment` strips it ITSELF, the defence that already survived a caller forgetting it once. The sentence is written generically because it has been incomplete twice. (5) **The strip has SEVEN cards**: `All returns` clears `segment` and is the default (unfiltered), the six segments sit beside it with `All open` among them. `All open` is a FILTER, not a clear — rendering it as the clear card would label closed, fully-refunded returns as open work. Only `restock_blocked` and `orphans` may render red (#2100's split); `all_open` is **neutral** precisely because it is the largest card on any active install, and tones are keyed off `MetricCard`'s own prop type so a local union cannot type-check against a tone the primitive does not render. (6) **The two rails are the existing per-line chips, extended — not a parallel component**: `ReturnLineStateChip`'s Wave-1c `tracked` prop was built as this seam and flips to default-true, gaining rail labels, the `in_doubt` warning with its *do not refund again* copy (OL crossed a boundary and does not know the outcome; a retry moves real money twice) and the `refunded` attribution. **`refunded` never renders without an observation** — `triggerRefund` writes `triggered` and only `recordRefundObservation` writes `refunded`, so `moneyState === 'refunded'` IS the observation and nothing derives one from the other, pinned by a `triggered`-only fixture. The standing *"these move independently"* sentence ships as COPY, once, above the table. `openedFrom`/`openedTo` filter **`openedAt`**, the SOURCE's own instant — never `createdAt`, which is OL's ingestion clock; wiring the spec's param to the existing arm would answer a question about the marketplace's timeline with OL's. The counters subquery gained the custody/money aggregates and a `RESTOCK_BLOCKED_EXISTS` whose predicate is `findOutstandingRestockEventsForReturn`'s verbatim — attestation FLIPS a blocked act's `restockState` to `handled_manually` rather than stamping it, so an `attestedByEventId IS NULL` clause would be backwards and keep every settled block forever. `joinCountersOnce` makes that join idempotent, because four paths can need it on one builder and a second `sc` alias is a TypeORM error on the ordinary "click a card, then narrow by stage" request. No migration.
- **The per-line receive and dispose flows (#2380, `W2-42`, spec § 5.2/§ 5.3)**: the acts the custody model exists to record. Wave 1c shipped the counters and #2376 their write API; both were inert from the UI until this. Six properties are decisions. (1) **Two of the issue's own controls had no backend, and were built rather than faked** — the standing rule that a control the backend cannot serve is dead code that type-checks. `markReturnCustodyNotReturned` was shipped, exported and **never called** (no service method, no route), and the detail read carried no way to name the restock destination at all. (2) **The destination is NAMED by the backend, never derived in the browser.** § 5.3 requires *"Stock will be added in {connection name}"*, and the name previously reached the client only on a `restockBlocked` response — i.e. after the write had already failed. `IReturnCustodyService.getRestockTarget()` answers from the SAME private resolver the dispose write uses, so reported === written-to structurally; a client-side pick over `enabledCapabilities` could not reproduce `listCapabilityAdapters`' candidate ordering and would confidently name a connection the write never touches. `ReturnRestockTarget`'s three non-resolved arms reuse the vocabulary a blocked restock records verbatim (`ambiguous-inventory-master` / `no-inventory-master` / `adapter-unresolved`), because a disclosure naming its states differently from the block it predicts is a second, drifting answer to one question — and `ambiguous-inventory-master` is a **blocked** restock, not a pick, so the UI must never render a candidate there. (3) **`Mark as not returned`, not "Mark remainder not returned".** The spec's phrasing describes a shortfall write the shipped model refuses: custody is single-valued per line, a line holding received units still needs a disposition for them, and there is no `quantityNotReturned` counter. The control is gated on `quantityReceived === 0` so the refusal is never discovered by clicking; where units did arrive the shortfall stays visible as `quantityAdvised - quantityReceived`, which is what the spec was reaching for. (4) **A guard ordered behind a broader one was dead, and replaced by a WRONG answer.** `partially-received` sat after the state check, but receiving units is exactly what moves a line to `received` — so no reachable line could emit it, and a half-delivered parcel was refused as *"already finished"*. Its unit test passed because it hand-built `{custodyState: 'in_transit', quantityReceived: 2}`, a combination the receipt transition cannot produce; an int-spec driving the real routes found it. The guards are reordered, the fixture now runs the real transition, and the shape is recorded in `docs/lessons.md`. (5) **A new `ReturnLineEventKind` member breaks nothing AND warns nobody** — the union has no `assertNever` switch, no exhaustive `Record`, and reaches persistence through an unchecked cast, so consumers are found by grep. `toLineEventKind` converts that silence into a warning while passing the stored value through **verbatim** (it is what the row says, not a lie); unlike the `readReturnReason` precedent beside it, `kind` has no honest default and `ReturnLineEvent.kind` is non-nullable, so an unrecognised value necessarily arrives typed as a member of a union it is not in. That is tolerable **only while nothing branches on `kind`**; the first `switch` or `Record` keyed on it makes the cast load-bearing and the entity-type widening stops being optional. (6) **The ≥44 px floor is explicit and width-keyed**, not inherited: the global rule fires under `@media (hover: none) and (pointer: coarse)` — a POINTER test — while both the acceptance criterion and § 5.2's declared style-guide departure frame the guarantee in terms of WIDTH, so a 768 px desktop window and every component test get nothing from it. A test strips media blocks by brace-counting and asserts the custody rules sit outside them. Also here: a `nothing-advised` refusal so a zero-advised line answers 409 with a code rather than a raw `CHK_return_line_events_quantity_positive` 500; `SegmentedControl` gains real `disabled` support (arrow-nav skips it, the tab stop never parks on it) so `Restock` can be shown-and-refused with its reason beside it rather than hidden; and the restock block is surfaced in place on the 2xx dispose response, because a stock write that silently no-ops is worse than none and § 5.4's full treatment is a sibling issue. No migration.
- **Deliberately NOT in this slice**: no `return.decline` (#2333), no read API (#2334), no restock execution, no HTTP mapping for `ReturnNotAttributedError` (#2334 owns the filter; unmapped, it surfaces as a 500), no manual operator re-attribution, no operator-facing backfill of a seller's pre-bootstrap return history, and no overlapped `createdAt.gte` repair re-sweep for SPIKE-2289 risk 2 (both named follow-ups). `shipments.direction` was likewise out of that slice and is now actioned — see § 23 below.


### 23. Shipment direction

- **A return label IS a shipment, so it shares the table — and the discriminator is what keeps every predicate honest (#2373, [ADR-060](./architecture/adrs/060-returns-aggregate-above-source-projection.md))**: same carrier, same waybill, same tracking poll, so a parallel `return_shipments` table would have duplicated every read predicate, the status machine and the `waybillRelayedAt` claim discipline. `shipments.direction` (`outbound | return`) is the column that lets one table hold both.
- **`direction` went into the branch-1 index's KEY columns, never its WHERE clause.** `UQ_shipments_branch_one_per_order_conn` is now `UNIQUE (orderId, connectionId, direction) WHERE "providerShipmentId" IS NULL`. Written instead as `direction = 'outbound'` in the predicate it would have kept guarding outbound rows and stopped guarding return rows **entirely** — any number of branch-1 return rows per `(order, connection)`. As a key column it is neither too narrow (it admits exactly the outbound+return pair ADR-060 needs) nor too wide (a second row in *either* direction is still refused). Verified against live Postgres, not inferred: an outbound and a return branch-1 row insert cleanly for one `(order, connection)`, while a second outbound and a second return are both rejected on `Key ("orderId", "connectionId", direction)`.
- **The column carries NO database default, and the migration's temporary one is a statement about history rather than a placeholder.** Migration `1862000000000` adds it `NOT NULL DEFAULT 'outbound'` and drops the default in the same `up()`. `'outbound'` is *true* of every existing row: `shipments` rows are written only by `ShipmentDispatchService` (an operator buying a label for goods going to a buyer) or by `FulfillmentStatusSyncService`'s branch-1 projection (a marketplace reporting the seller shipped), and no code path in the tree has ever been able to create a return label. Dropping the default is what stops it becoming an implicit answer for future inserts — left in place, TypeORM's schema sync would keep re-adding it and `direction` would be unstated-but-present. The ORM entity declares no default either, so the migration and `synchronize` (which the integration harness uses instead of migrations) build the identical shape.
- **Three read predicates take a REQUIRED `direction`; the fourth takes an optional filter, and the asymmetry is the point.** `findByOrderId` / `findActiveByOrderId` / `findBranchOneByOrderAndConnection` require it because a default would be a *silent decline* — a later call site would read the outbound cohort while believing it read every shipment, and nothing would say so; required, omission is a compile error. `findMany` instead gains an optional `ShipmentFilters.direction`, because it also backs the operator-facing `GET /shipments` list, where a hard outbound filter would make return labels permanently invisible with no signal. The status-sync scan therefore passes `'outbound'` **explicitly** rather than inheriting it, and the list API exposes `direction` as both a filter and a response field so the operator sees the cohort instead of having it chosen for them. `findBranchOneByOrderAndConnection` additionally *must* carry it, or the lookup would match a sibling row the widened index deliberately permits.
- **Nothing writes `'return'` yet.** Buying a return label is a later slice; this one gives it a column and an index that can hold it, exercised by the D8 coexistence regression test. The rollback is consequently safe today and conditionally destructive later — recreating the two-column index fails on any order holding both cohorts — which is recorded in the migration's own `down()`.
...
interface InventoryMasterPort {
   ...
   *
   * The command may carry an optional `idempotencyKey` + closed `reason`, and
   * the result an optional `adjustmentOutcome` (#2368) — see § Inventory.
   */
...
`returns` (#2327, § 22) is deliberately **not** a fourth, and each slice has moved it further from the property rather than closer: it now carries **six** real outbound edges. `returns --> identifier-mapping` (#2328) is a NestJS module edge — `ReturnsService` resolves a source-native order id to an OL internal one through `IIdentifierMappingService`. `returns --> integrations` and `returns --> sync` (#2330) are likewise module edges: the two ingestion services resolve the connection's `OrderSource` adapter, and take the cursor seam, the job queue and the lock. All three are acyclic — none of those modules imports this one. `returns --> orders` began type-only (the `RefundReason` vocabulary off the `@openlinker/core/orders/types` cycle-breaker sub-barrel), became a real **value** edge with #2330's `isReturnSourceReader` guard, and as of **#2333 is a real NestJS module edge**: `ReturnsModule` imports `OrderChangesModule` — deliberately that leaf module rather than `OrdersModule` — and `ReturnDeclineService` injects `ORDER_CHANGE_SERVICE_TOKEN`. **`returns --> invoicing` is the fifth (#2374)** — `ReturnCorrectionProposalService` reads the issued-line snapshot through `IInvoiceService`, and it too is acyclic (`InvoicingModule` imports only `IntegrationsModule` / `IdentifierMappingModule` / `SyncModule` / `SalesDocumentsModule`) and strictly READ-ONLY: nothing on that path issues a document. **`returns --> fulfillment-authority` is the sixth (#2352)** — a real **value** edge: `ReturnRepository` value-imports `readAuthorityAttentionEntries` to coerce the `omsAttention` column at the mapping boundary, and `ReturnRecord` takes `AuthorityAttentionEntry` as its last constructor parameter. It is the inbound half of the edge the paragraph above records from `fulfillment-authority`'s own side, and it is acyclic for that context's defining reason: the leaf imports no sibling, so nothing points back. Registering the context in `ZERO_SIBLING_EDGE_LEAVES` would be a false claim rather than a tidy one.
