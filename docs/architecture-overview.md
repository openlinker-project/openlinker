...
- **Deliberately NOT in this slice**: no service (#2328), no return feed / `ReturnSourceReader` (#2329), no `return.decline` (#2333), no read API (#2334), no restock execution, and **no `shipments.direction`** — return labels are Wave 2, so ADR-060's note that `UQ_shipments_branch_one_per_order_conn` must gain `direction` in its predicate is carried forward rather than actioned.
...
`returns` (#2327, § 22) is deliberately **not** a fourth: its single outbound edge — `returns --> orders`, for the `RefundReason` vocabulary it reuses verbatim — is real, so the property the table asserts is simply not true of it, and registering it would be a false claim rather than a tidy one. It does, however, share the leaves' **root-barrel** posture for the same cycle-safety reason, and the one-way discipline matters just as much in its case: `orders` must never import `returns` back.
